...
bool TxGetSubsciberData(TATP *tatp_client, uint64_t *seed, coro_yield_t &yield,
                        tx_id_t tx_id, TXN *txn, GetSubscriberTxnParam *param) {
  // RDMA_LOG(DBG) << "coro " << txn->coro_id << " executes TxGetSubsciberData,
  // tx_id=" << tx_id;
  ...
  sub_key.s_id = param->s_id_;

  // This empty data sub_record will be filled by RDMA reading from remote when
  // running transaction
  auto sub_record = std::make_shared<DataSetItem>(
      (table_id_t)TATPTableType::kSubscriberTable, tatp_sub_val_t_size,
      sub_key.item_key, UserOP::kRead);
  ...
  if (value->msc_location != tatp_sub_msc_location_magic) {
    RDMA_LOG(DBG) << "my msc_location: " << value->msc_location
                  << " vs location_magic: " << tatp_sub_msc_location_magic;
    sub_record->Debug();
...
bool TxGetNewDestination(TATP *tatp_client, uint64_t *seed, coro_yield_t &yield,
                         tx_id_t tx_id, TXN *txn,
                         GetNewDestinationTxnParam *param) {
  // RDMA_LOG(DBG) << "coro " << txn->coro_id << " executes
  // TxGetNewDestination";
  ...
  uint32_t s_id = param->s_id_;
  uint8_t sf_type = param->sf_type_;
  uint8_t start_time = param->start_time_;
  uint8_t end_time = param->end_time_;
  ...
  auto specfac_record = std::make_shared<DataSetItem>(
      (table_id_t)TATPTableType::kSpecialFacilityTable, tatp_specfac_val_t_size,
      specfac_key.item_key, UserOP::kRead);
  ...
  for (unsigned i = 0; i < cf_to_fetch; i++) {
    ...
    callfwd_record[i] = std::make_shared<DataSetItem>(
        (table_id_t)TATPTableType::kCallForwardingTable,
        tatp_callfwd_val_t_size, callfwd_key[i].item_key, UserOP::kRead);
...
bool TxGetAccessData(TATP *tatp_client, uint64_t *seed, coro_yield_t &yield,
                     tx_id_t tx_id, TXN *txn, GetAccessDataTxnParam *param) {
  // RDMA_LOG(DBG) << "coro " << txn->coro_id << " executes TxGetAccessData,
  // tx_id=" << tx_id;
  ...
  key.s_id = param->s_id_;
  key.ai_type = param->ai_type_;

  auto acc_record = std::make_shared<DataSetItem>(
      (table_id_t)TATPTableType::kAccessInfoTable, tatp_accinf_val_t_size,
      key.item_key, UserOP::kRead);
...
bool TxUpdateSubscriberData(TATP *tatp_client, uint64_t *seed,
  ...
  auto sub_record = std::make_shared<DataSetItem>(
      (table_id_t)TATPTableType::kSubscriberTable, tatp_sub_val_t_size,
      sub_key.item_key, UserOP::kUpdate);
  ...
  auto specfac_record = std::make_shared<DataSetItem>(
      (table_id_t)TATPTableType::kSpecialFacilityTable, tatp_specfac_val_t_size,
      specfac_key.item_key, UserOP::kUpdate);
  ...
  sub_record->SetUpdate(tatp_sub_val_bitmap::bits, &sub_val->bits,
                        sizeof(sub_val->bits));
  ...
  specfac_record->SetUpdate(tatp_specfac_val_bitmap::data_a,
                            &specfac_val->data_a, sizeof(specfac_val->data_a));
...
bool TxUpdateLocation(TATP *tatp_client, uint64_t *seed, coro_yield_t &yield,
                      tx_id_t tx_id, TXN *txn, UpdateLocationTxnParam *param) {
  ...
  uint32_t s_id = param->s_id_;
  uint32_t vlr_location = param->vlr_location_;
  ...
  auto sec_sub_record = std::make_shared<DataSetItem>(
      (table_id_t)TATPTableType::kSecSubscriberTable, tatp_sec_sub_val_t_size,
      sec_sub_key.item_key, UserOP::kRead);
  ...
  sub_record->SetUpdate(tatp_sub_val_bitmap::vlr_location,
                        &sub_val->vlr_location, sizeof(sub_val->vlr_location));
...
bool TxInsertCallForwarding(TATP *tatp_client, uint64_t *seed,
  ...
  auto callfwd_record = std::make_shared<DataSetItem>(
      (table_id_t)TATPTableType::kCallForwardingTable,
      tatp_callfwd_val_t_size, // Here Insert
      callfwd_key.item_key, UserOP::kInsert);
  ...
  if (!callfwd_record->IsRealInsert()) {
    callfwd_record->SetUpdate(tatp_callfwd_val_bitmap::end_time,
                              &callfwd_val->end_time,
                              sizeof(callfwd_val->end_time));
    callfwd_record->SetUpdate(tatp_callfwd_val_bitmap::numberx,
                              callfwd_val->numberx,
                              sizeof(callfwd_val->numberx));
  }
...
bool TxDeleteCallForwarding(TATP *tatp_client, uint64_t *seed,
  ...
  uint32_t s_id = tatp_client->GetNonUniformRandomSubscriber(seed);
  uint8_t sf_type = (FastRand(seed) % 4);
  uint8_t start_time = (FastRand(seed) % 3) * 8;
  ...
  auto callfwd_record = std::make_shared<DataSetItem>(
      (table_id_t)TATPTableType::kCallForwardingTable, tatp_callfwd_val_t_size,
      callfwd_key.item_key, UserOP::kDelete);
  txn->AddToReadWriteSet(callfwd_record);
