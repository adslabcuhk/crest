...
void TATP::LoadTable(node_id_t node_id, node_id_t num_server,
...

  {
    ...
    subscriber_table = new HashStore(
        (table_id_t)TATPTableType::kSubscriberTable,
        table_config.get("num_subscriber").get_uint64(), mem_store_alloc_param);
    ...
    std::cerr << "SUBSCRIBER max occupy slot num: "
              << subscriber_table->GetMaxOccupySlotNum() << std::endl;
  }

  {
    ...
    std::cerr << "SECONDARY SUBSCRIBER max occupy slot num: "
              << sec_subscriber_table->GetMaxOccupySlotNum() << std::endl;
  }

  {
    ...
    std::cerr << "ACCESS INFO max occupy slot num: "
              << access_info_table->GetMaxOccupySlotNum() << std::endl;
  }

  {
    RDMA_LOG(DBG) << "Loading SPECIAL FACILITY+CALL FORWARDING table";
    special_facility_table =
        new HashStore((table_id_t)TATPTableType::kSpecialFacilityTable,
    ...
    std::cerr << "SPECIAL FACILITY max occupy slot num: "
              << special_facility_table->GetMaxOccupySlotNum() << std::endl;
    std::cerr << "CALL FORWARDING max occupy slot num: "
              << call_forwarding_table->GetMaxOccupySlotNum() << std::endl;
  }
  ...
  if ((node_id_t)TATPTableType::kSubscriberTable % num_server == node_id) {
    RDMA_LOG(EMPH) << "[Primary] SUBSCRIBER table ID: "
                   << (node_id_t)TATPTableType::kSubscriberTable;
    std::cerr << "Number of initial records: " << std::dec
              << subscriber_table->GetInitInsertNum() << std::endl;
    primary_table_ptrs.push_back(subscriber_table);
  }

  if ((node_id_t)TATPTableType::kSecSubscriberTable % num_server == node_id) {
    RDMA_LOG(EMPH) << "[Primary] SECONDARY SUBSCRIBER table ID: "
                   << (node_id_t)TATPTableType::kSecSubscriberTable;
    std::cerr << "Number of initial records: " << std::dec
              << sec_subscriber_table->GetInitInsertNum() << std::endl;
    primary_table_ptrs.push_back(sec_subscriber_table);
  }

  if ((node_id_t)TATPTableType::kAccessInfoTable % num_server == node_id) {
    RDMA_LOG(EMPH) << "[Primary] ACCESS INFO table ID: "
                   << (node_id_t)TATPTableType::kAccessInfoTable;
    std::cerr << "Number of initial records: " << std::dec
              << access_info_table->GetInitInsertNum() << std::endl;
    primary_table_ptrs.push_back(access_info_table);
  }

  if ((node_id_t)TATPTableType::kSpecialFacilityTable % num_server == node_id) {
    RDMA_LOG(EMPH) << "[Primary] SPECIAL FACILITY+CALL FORWARDING table IDs: "
                   << (node_id_t)TATPTableType::kSpecialFacilityTable << " + "
                   << (node_id_t)TATPTableType::kCallForwardingTable;
    std::cerr << "Number of initial records: " << std::dec
              << special_facility_table->GetInitInsertNum() << std::endl;
    std::cerr << "Number of initial records: " << std::dec
              << call_forwarding_table->GetInitInsertNum() << std::endl;
  ...
  if (BACKUP_NUM < num_server) {
    for (node_id_t i = 1; i <= BACKUP_NUM; i++) {
      if ((node_id_t)TATPTableType::kSubscriberTable % num_server ==
          (node_id - i + num_server) % num_server) {
        // Meaning: I (current node_id) am the backup-SubscriberTable of my
        // primary. My primary-SubscriberTable resides on a node, whose id is
        // TATPTableType::kSubscriberTable % num_server A possible layout: | P
        // (My primary) | B1 (I'm here) | B2 (Or I'm here) |
        RDMA_LOG(DBG) << "[Backup] SUBSCRIBER table ID: "
                      << (node_id_t)TATPTableType::kSubscriberTable;
        std::cerr << "Number of initial records: " << std::dec
                  << subscriber_table->GetInitInsertNum() << std::endl;
      ...
      if ((node_id_t)TATPTableType::kSecSubscriberTable % num_server ==
          (node_id - i + num_server) % num_server) {
        RDMA_LOG(DBG) << "[Backup] SECONDARY SUBSCRIBER table ID: "
                      << (node_id_t)TATPTableType::kSecSubscriberTable;
        std::cerr << "Number of initial records: " << std::dec
                  << sec_subscriber_table->GetInitInsertNum() << std::endl;
      ...
      if ((node_id_t)TATPTableType::kAccessInfoTable % num_server ==
          (node_id - i + num_server) % num_server) {
        RDMA_LOG(DBG) << "[Backup] ACCESS INFO table ID: "
                      << (node_id_t)TATPTableType::kAccessInfoTable;
        std::cerr << "Number of initial records: " << std::dec
                  << access_info_table->GetInitInsertNum() << std::endl;
      ...
      if ((node_id_t)TATPTableType::kSpecialFacilityTable % num_server ==
          (node_id - i + num_server) % num_server) {
        RDMA_LOG(DBG) << "[Backup] SPECIAL FACILITY+CALL FORWARDING table IDs: "
                      << (node_id_t)TATPTableType::kSpecialFacilityTable
                      << " + "
                      << (node_id_t)TATPTableType::kCallForwardingTable;
        std::cerr << "Number of initial records: " << std::dec
...
void TATP::PopulateSubscriberTable() {
  ...
  for (uint32_t s_id = 0; s_id < subscriber_size; s_id++) {
    ...
    LoadRecord(subscriber_table, key.item_key, (void *)&sub_val,
...
void TATP::PopulateSecondarySubscriberTable() {
  ...
  for (uint32_t s_id = 0; s_id < subscriber_size; s_id++) {
    ...
    LoadRecord(sec_subscriber_table, key.item_key, (void *)&sec_sub_val,
...
void TATP::PopulateAccessInfoTable() {
  ...
  for (uint32_t s_id = 0; s_id < subscriber_size; s_id++) {
    ...
    for (uint8_t ai_type : ai_type_vec) {
      ...
      LoadRecord(access_info_table, key.item_key, (void *)&accinf_val,
...
void TATP::PopulateSpecfacAndCallfwdTable() {
  std::vector<uint8_t> sf_type_values = {0, 1, 2, 3};
  ...
  /* Populate the tables */
  for (uint32_t s_id = 0; s_id < subscriber_size; s_id++) {
    std::vector<uint8_t> sf_type_vec = sf_type_values;
        // SelectUniqueItem(&tmp_seed, sf_type_values, 1, 4);
    ...
    for (uint8_t sf_type : sf_type_vec) {
      ...
      specfac_val.is_active = (FastRand(&tmp_seed) % 100 < 100) ? 1 : 0;
      LoadRecord(special_facility_table, key.item_key, (void *)&specfac_val,
      ...
      for (size_t start_time = 0; start_time <= 16; start_time += 8) {
        ...
        callfwd_val.end_time = 24;
        LoadRecord(call_forwarding_table, key.item_key, (void *)&callfwd_val,
...
void TATP::LoadRecord(HashStore *table, itemkey_t item_key, void *val_ptr,
 ...
 */
std::vector<uint8_t> TATP::SelectUniqueItem(uint64_t *tmp_seed,
