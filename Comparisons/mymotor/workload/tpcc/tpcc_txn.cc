...
bool TxNewOrder(TPCC *tpcc_client, FastRandom *random_generator,
  ...
  */
  ...
  txn->Begin(tx_id, TXN_TYPE::kRWTxn, "TxNewOrder");
  ...
  if (!param->valid) {
    ...
    warehouse_id = tpcc_client->PickWarehouseId(
        random_generator[0], warehouse_id_start_, warehouse_id_end_);
    district_id = tpcc_client->RandomNumber(
        random_generator[0], district_id_start, district_id_end_);
    customer_id = tpcc_client->GetCustomerId(random_generator[0]);
    c_key =
        tpcc_client->MakeCustomerKey(warehouse_id, district_id, customer_id);

    std::set<uint64_t> stock_set; // remove identity stock ids;

    num_items = tpcc_client->RandomNumber(random_generator[0],
                                          tpcc_order_line_val_t::MIN_OL_CNT,
                                          tpcc_order_line_val_t::MAX_OL_CNT);

    for (int i = 0; i < num_items; i++) {
      int64_t item_id = tpcc_client->GetItemId(random_generator[0]);
      if (tpcc_client->num_warehouse == 1 ||
          tpcc_client->RandomNumber(random_generator[0], 1, 100) >
              g_new_order_remote_item_pct) {
        // local stock case
        uint32_t supplier_warehouse_id = warehouse_id;
        int64_t s_key =
            tpcc_client->MakeStockKey(supplier_warehouse_id, item_id);
        ...
        do {
          supplier_warehouse_id = tpcc_client->RandomNumber(
              random_generator[0], 1, tpcc_client->num_warehouse);
    ...
    // Store these parameters to the input Param:
    param->warehouse_id_ = warehouse_id;
    param->district_id_ = district_id;
    param->customer_id_ = customer_id;
    param->num_items = num_items;
    param->num_local_stocks = num_local_stocks;
    param->num_remote_stocks = num_remote_stocks;
    param->all_local = all_local;
    memcpy(param->remote_stocks, remote_stocks, sizeof(remote_stocks));
    memcpy(param->local_stocks, local_stocks, sizeof(local_stocks));
    memcpy(param->remote_item_ids, remote_item_ids, sizeof(remote_item_ids));
    memcpy(param->local_item_ids, local_item_ids, sizeof(local_item_ids));
    memcpy(param->remote_supplies, remote_supplies, sizeof(remote_supplies));
    memcpy(param->local_supplies, local_supplies, sizeof(local_supplies));
  } else {
    // Recover the parameter from the input Param:
    warehouse_id = param->warehouse_id_;
    district_id = param->district_id_;
    customer_id = param->customer_id_;
    c_key =
        tpcc_client->MakeCustomerKey(warehouse_id, district_id, customer_id);
    num_items = param->num_items;
    num_local_stocks = param->num_local_stocks;
    num_remote_stocks = param->num_remote_stocks;
    all_local = param->all_local;
    memcpy(remote_stocks, param->remote_stocks, sizeof(remote_stocks));
    memcpy(local_stocks, param->local_stocks, sizeof(local_stocks));
    memcpy(remote_item_ids, param->remote_item_ids, sizeof(remote_item_ids));
    memcpy(local_item_ids, param->local_item_ids, sizeof(local_item_ids));
    memcpy(remote_supplies, param->remote_supplies, sizeof(remote_supplies));
    memcpy(local_supplies, param->local_supplies, sizeof(local_supplies));
  }
  ...
  auto ware_record = std::make_shared<DataSetItem>(
      (table_id_t)TPCCTableType::kWarehouseTable, tpcc_warehouse_val_t_size,
      ware_key.item_key, UserOP::kRead);
  txn->AddToReadOnlySet(ware_record);

  tpcc_customer_key_t cust_key;
  cust_key.c_id = c_key;
  auto cust_record = std::make_shared<DataSetItem>(
      (table_id_t)TPCCTableType::kCustomerTable, tpcc_customer_val_t_size,
      cust_key.item_key, UserOP::kRead);
  ...
  dist_key.d_id = d_key;
  auto dist_record = std::make_shared<DataSetItem>(
      (table_id_t)TPCCTableType::kDistrictTable, tpcc_district_val_t_size,
      dist_key.item_key, UserOP::kUpdate);
  txn->AddToReadWriteSet(dist_record);

  std::vector<DataSetItemPtr> item_records;
  std::vector<DataSetItemPtr> stock_records;
  std::vector<DataSetItemPtr> orderline_records;

  std::vector<DataSetItemPtr> remote_item_records;
  std::vector<DataSetItemPtr> remote_stock_records;
  std::vector<DataSetItemPtr> remote_orderline_records;

  // -----------------------------------------------------------------------------
  // (kqh): We modified the logic how NewOrder transactions are executed in
  // Motor: The transaction first prepare all the records, then call the
  // "Execute" to acuqire all records
  for (int ol_number = 1; ol_number <= num_local_stocks; ol_number++) {
    ...
    const uint32_t ol_quantity =
        tpcc_client->RandomNumber(random_generator[0], 1, 10);
    // read item info
    tpcc_item_key_t tpcc_item_key;
    tpcc_item_key.i_id = ol_i_id;

    auto item_record = std::make_shared<DataSetItem>(
        (table_id_t)TPCCTableType::kItemTable, tpcc_item_val_t_size,
        tpcc_item_key.item_key, UserOP::kRead);
    txn->AddToReadOnlySet(item_record);
    item_records.push_back(item_record);
    ...
    auto stock_record = std::make_shared<DataSetItem>(
        (table_id_t)TPCCTableType::kStockTable, tpcc_stock_val_t_size,
        stock_key.item_key, UserOP::kUpdate);
    txn->AddToReadWriteSet(stock_record);
    stock_records.push_back(stock_record);
  }
  ...
  for (int ol_number = 1; ol_number <= num_remote_stocks; ol_number++) {
    ...
    txn->AddToReadOnlySet(item_record);
    remote_item_records.push_back(item_record);
    ...
    auto stock_record = std::make_shared<DataSetItem>(
        ...
        stock_key.item_key, UserOP::kUpdate);
    txn->AddToReadWriteSet(stock_record);
    remote_stock_records.push_back(stock_record);
  }
  ...
  if (check != tpcc_zip_magic) {
    RDMA_LOG(FATAL) << "[FATAL] Read warehouse unmatch, tid-cid-txid: "
                    << txn->t_id << "-" << txn->coro_id << "-" << tx_id;
  }

  auto *cust_val = (tpcc_customer_val_t *)cust_record->Value();
  // c_since never be 0
  if (cust_val->c_since == 0) {
    RDMA_LOG(FATAL) << "[FATAL] Read customer unmatch, tid-cid-txid: "
                    << txn->t_id << "-" << txn->coro_id << "-" << tx_id;
  }
  ...
  if (check != tpcc_zip_magic) {
    RDMA_LOG(FATAL) << "[FATAL] Read district unmatch, tid-cid-txid-key: "
                    << txn->t_id << "-" << txn->coro_id << "-" << tx_id << "-"
                    << dist_key.item_key << " read d_zip is: " << check;
  }

  const auto my_next_o_id = dist_val->d_next_o_id;

  dist_record->SetUpdate(tpcc_district_val_bitmap::d_next_o_id,
                         &dist_val->d_next_o_id, sizeof(dist_val->d_next_o_id));
  ...
  auto norder_record =
      std::make_shared<DataSetItem>((table_id_t)TPCCTableType::kNewOrderTable,
                                    tpcc_new_order_val_t_size, // Insert
                                    norder_key.item_key, UserOP::kInsert);
  ...
  auto order_record =
      std::make_shared<DataSetItem>((table_id_t)TPCCTableType::kOrderTable,
                                    tpcc_order_val_t_size, // Insert
                                    order_key.item_key, UserOP::kInsert);
  ...
  // auto oidx_record =
  //     std::make_shared<DataSetItem>((table_id_t)TPCCTableType::kOrderIndexTable,
  ...
  for (int ol_number = 1; ol_number <= num_remote_stocks; ol_number++) {
    ...
    // ol_key;
    auto ol_record = std::make_shared<DataSetItem>(
        (table_id_t)TPCCTableType::kOrderLineTable,
        sizeof(tpcc_order_line_val_t), // Insert
        order_line_key.item_key, UserOP::kInsert);
    txn->AddToReadWriteSet(ol_record);
    remote_orderline_records.push_back(ol_record);
  }
  ...
  for (int ol_number = 1; ol_number <= num_local_stocks; ol_number++) {
    ...
    auto ol_record = std::make_shared<DataSetItem>(
        ...
        order_line_key.item_key, UserOP::kInsert);
    txn->AddToReadWriteSet(ol_record);
    orderline_records.push_back(ol_record);
  }
  ...
  tpcc_new_order_val_t *norder_val =
      (tpcc_new_order_val_t *)norder_record->Value();
  if (!norder_record->IsRealInsert()) {
    norder_record->SetUpdate(tpcc_new_order_val_bitmap::no_dummy,
                             norder_val->no_dummy,
                             sizeof(norder_val->no_dummy));
  }
  ...
  if (!order_record->IsRealInsert()) {
    order_record->SetUpdate(tpcc_order_val_bitmap::o_c_id, &order_val->o_c_id,
                            sizeof(order_val->o_c_id));
    order_record->SetUpdate(tpcc_order_val_bitmap::o_carrier_id,
                            &order_val->o_carrier_id,
                            sizeof(order_val->o_carrier_id));
    order_record->SetUpdate(tpcc_order_val_bitmap::o_ol_cnt,
                            &order_val->o_ol_cnt, sizeof(order_val->o_ol_cnt));
    order_record->SetUpdate(tpcc_order_val_bitmap::o_all_local,
                            &order_val->o_all_local,
                            sizeof(order_val->o_all_local));
    order_record->SetUpdate(tpcc_order_val_bitmap::o_entry_d,
                            &order_val->o_entry_d,
                            sizeof(order_val->o_entry_d));
  }
  ...
  //     (tpcc_order_index_val_t *)oidx_record->Value();
  // if (!oidx_record->IsRealInsert()) {
  //   oidx_record->SetUpdate(tpcc_order_index_val_bitmap::o_id,
  //   &oidx_val->o_id,
  //                          sizeof(oidx_val->o_id));
  // }
  // oidx_val->o_id = o_key;
  // oidx_val->debug_magic = tpcc_add_magic;

  for (int ol_number = 1; ol_number <= num_local_stocks; ol_number++) {
    auto item_record = item_records[ol_number - 1];
    auto stock_record = stock_records[ol_number - 1];
    auto ol_record = orderline_records[ol_number - 1];
    ...
    stock_record->SetUpdate(tpcc_stock_val_bitmap::s_quantity,
                            &stock_val->s_quantity,
                            sizeof(stock_val->s_quantity));
    ...
    stock_record->SetUpdate(tpcc_stock_val_bitmap::s_ytd, &stock_val->s_ytd,
                            sizeof(stock_val->s_ytd));
    stock_val->s_ytd += ol_quantity;

    stock_record->SetUpdate(tpcc_stock_val_bitmap::s_remote_cnt,
                            &stock_val->s_remote_cnt,
                            sizeof(stock_val->s_remote_cnt));
    stock_val->s_remote_cnt +=
    ...
    tpcc_order_line_val_t *order_line_val =
        (tpcc_order_line_val_t *)ol_record->Value();

    if (!ol_record->IsRealInsert()) {
      ol_record->SetUpdate(tpcc_order_line_val_bitmap::ol_i_id,
                           &order_line_val->ol_i_id,
                           sizeof(order_line_val->ol_i_id));
      ol_record->SetUpdate(tpcc_order_line_val_bitmap::ol_delivery_d,
                           &order_line_val->ol_delivery_d,
                           sizeof(order_line_val->ol_delivery_d));
      ol_record->SetUpdate(tpcc_order_line_val_bitmap::ol_amount,
                           &order_line_val->ol_amount,
                           sizeof(order_line_val->ol_amount));
      ol_record->SetUpdate(tpcc_order_line_val_bitmap::ol_supply_w_id,
                           &order_line_val->ol_supply_w_id,
                           sizeof(order_line_val->ol_supply_w_id));
      ol_record->SetUpdate(tpcc_order_line_val_bitmap::ol_quantity,
                           &order_line_val->ol_quantity,
                           sizeof(order_line_val->ol_quantity));
    }
  ...
  for (int ol_number = 1; ol_number <= num_remote_stocks; ol_number++) {
    auto item_record = remote_item_records[ol_number - 1];
    auto stock_record = remote_stock_records[ol_number - 1];
    auto ol_record = remote_orderline_records[ol_number - 1];
...
bool TxPayment(TPCC *tpcc_client, FastRandom *random_generator,
  ...
  txn->Begin(tx_id, TXN_TYPE::kRWTxn, "TxPayment");

  uint32_t warehouse_id = 0, district_id = 0;
  int32_t c_w_id;
  int32_t c_d_id;
  ...
  if (!param->valid) {
    // Generate parameters
    int x = tpcc_client->RandomNumber(random_generator[0], 1, 100);
    int y = tpcc_client->RandomNumber(random_generator[0], 1, 100);
    ...
    if (tpcc_client->num_warehouse == 1 || x <= 85) {
      ...
      do {
        c_w_id = tpcc_client->RandomNumber(random_generator[0], 1,
                                           tpcc_client->num_warehouse);
      } while (c_w_id == warehouse_id);
      c_d_id = tpcc_client->RandomNumber(random_generator[0], district_id_start,
                                         district_id_end_);
    }
    // The payment amount (H_AMOUNT) is randomly selected within [1.00 ..
    // 5,000.00].
    h_amount =
        (float)tpcc_client->RandomNumber(random_generator[0], 100, 500000) /
        100.0;
    if (y <= 60) {
      // 60%: payment by last name
      char last_name[tpcc_customer_val_t::MAX_LAST + 1];
      size_t size =
          (tpcc_client->GetNonUniformCustomerLastNameLoad(random_generator[0]))
              .size();
      ...
      // are selected sorted by C_FIRST in ascending order. Let n be the number
      // of rows selected. C_ID, C_FIRST, C_MIDDLE, C_STREET_1, C_STREET_2,
      // C_CITY, C_STATE, C_ZIP, C_PHONE, C_SINCE, C_CREDIT, C_CREDIT_LIM,
      // C_DISCOUNT, and C_BALANCE are retrieved from the row at position (n/ 2
      // rounded up to the next integer) in the sorted set of selected rows from
      // the CUSTOMER table.
      customer_id = tpcc_client->GetCustomerId(random_generator[0]);
    } else {
      // 40%: payment by id
    ...
    // Store this parameter for retry execution
    param->warehouse_id_ = warehouse_id;
    param->district_id_ = district_id;
    param->customer_id_ = customer_id;
    param->c_w_id_ = c_w_id;
    param->c_d_id_ = c_d_id;
    param->h_amount_ = h_amount;
  } else {
    // Recover the parameters
    warehouse_id = param->warehouse_id_;
    district_id = param->district_id_;
    customer_id = param->customer_id_;
    c_w_id = param->c_w_id_;
    c_d_id = param->c_d_id_;
    h_amount = param->h_amount_;
  }
  ...
  auto ware_record = std::make_shared<DataSetItem>(
      (table_id_t)TPCCTableType::kWarehouseTable, tpcc_warehouse_val_t_size,
      ware_key.item_key, UserOP::kUpdate);
  ...
  auto cust_record = std::make_shared<DataSetItem>(
      (table_id_t)TPCCTableType::kCustomerTable, tpcc_customer_val_t_size,
      cust_key.item_key, UserOP::kUpdate);
  ...
  hist_key.h_id = tpcc_client->MakeHistoryKey(warehouse_id, district_id, c_w_id,
                                              c_d_id, customer_id);
  auto hist_record =
      std::make_shared<DataSetItem>((table_id_t)TPCCTableType::kHistoryTable,
                                    tpcc_history_val_t_size, // Insert
                                    hist_key.item_key, UserOP::kInsert);
  ...
  ware_record->SetUpdate(tpcc_warehouse_val_bitmap::w_ytd, &ware_val->w_ytd,
                         sizeof(ware_val->w_ytd));
  ware_val->w_ytd += h_amount;

  dist_record->SetUpdate(tpcc_district_val_bitmap::d_ytd, &dist_val->d_ytd,
                         sizeof(dist_val->d_ytd));
  dist_val->d_ytd += h_amount;

  cust_record->SetUpdate(tpcc_customer_val_bitmap::c_balance,
                         &cust_val->c_balance, sizeof(cust_val->c_balance));
  cust_val->c_balance -= h_amount;

  cust_record->SetUpdate(tpcc_customer_val_bitmap::c_ytd_payment,
                         &cust_val->c_ytd_payment,
                         sizeof(cust_val->c_ytd_payment));
  cust_val->c_ytd_payment += h_amount;

  cust_record->SetUpdate(tpcc_customer_val_bitmap::c_payment_cnt,
                         &cust_val->c_payment_cnt,
                         sizeof(cust_val->c_payment_cnt));
  ...
  if (strcmp(cust_val->c_credit, BAD_CREDIT) == 0) {
    ...
    cust_record->SetUpdate(tpcc_customer_val_bitmap::c_data, cust_val->c_data,
                           sizeof(cust_val->c_data));
  ...
  if (!hist_record->IsRealInsert()) {
    hist_record->SetUpdate(tpcc_history_val_bitmap::h_date, &hist_val->h_date,
                           sizeof(hist_val->h_date));
    hist_record->SetUpdate(tpcc_history_val_bitmap::h_amount,
                           &hist_val->h_amount, sizeof(hist_val->h_amount));
    hist_record->SetUpdate(tpcc_history_val_bitmap::h_data, hist_val->h_data,
                           sizeof(hist_val->h_data));
  }
  hist_val->h_date =
...
bool TxDelivery(TPCC *tpcc_client, FastRandom *random_generator,
  ...
  */
  ...
  txn->Begin(tx_id, TXN_TYPE::kRWTxn, "TxDelivery");
  ...
  const uint32_t warehouse_id = tpcc_client->PickWarehouseId(
      random_generator[txn->coro_id], warehouse_id_start_, warehouse_id_end_);
  const int o_carrier_id = tpcc_client->RandomNumber(
      random_generator[txn->coro_id], tpcc_order_val_t::MIN_CARRIER_ID,
  ...
  for (int d_id = 1; d_id <= tpcc_client->num_district_per_warehouse; d_id++) {
    ...
    int o_id = tpcc_client->RandomNumber(random_generator[txn->coro_id],
                                         min_o_id, max_o_id);

    int64_t no_key = tpcc_client->MakeNewOrderKey(warehouse_id, d_id, o_id);
    tpcc_new_order_key_t norder_key;
    norder_key.no_id = no_key;
    auto norder_record_try_read = std::make_shared<DataSetItem>(
        (table_id_t)TPCCTableType::kNewOrderTable, tpcc_new_order_val_t_size,
        norder_key.item_key, UserOP::kRead);
    ...
    // The new order record exists. Remove the new order record from read only
    // set
    txn->RemoveLastROItem();

    // Add the new order record to read write set to be deleted
    auto norder_record = std::make_shared<DataSetItem>(
        (table_id_t)TPCCTableType::kNewOrderTable, tpcc_new_order_val_t_size,
        norder_key.item_key, UserOP::kDelete);
    txn->AddToReadWriteSet(norder_record);

    uint64_t o_key = tpcc_client->MakeOrderKey(warehouse_id, d_id, o_id);
    tpcc_order_key_t order_key;
    order_key.o_id = o_key;
    auto order_record = std::make_shared<DataSetItem>(
        (table_id_t)TPCCTableType::kOrderTable, tpcc_order_val_t_size,
        order_key.item_key, UserOP::kUpdate);
    txn->AddToReadWriteSet(order_record);
    ...
    if (!norder_record->is_delete_no_read_value) {
      if (no_val->debug_magic != tpcc_add_magic) {
        RDMA_LOG(FATAL) << "[FATAL] Read new order unmatch, tid-cid-txid: "
                        << txn->t_id << "-" << txn->coro_id << "-" << tx_id;
      }
    }
    ...
    if (order_val->o_entry_d == 0) {
      RDMA_LOG(FATAL) << "[FATAL] Read order unmatch, tid-cid-txid: "
                      << txn->t_id << "-" << txn->coro_id << "-" << tx_id;
    }
    ...
    for (int line_number = 1; line_number <= tpcc_order_line_val_t::MAX_OL_CNT;
      ...
      tpcc_order_line_key_t order_line_key;
      order_line_key.ol_id = ol_key;
      auto ol_record_try_read = std::make_shared<DataSetItem>(
          (table_id_t)TPCCTableType::kOrderLineTable,
          tpcc_order_line_val_t_size, order_line_key.item_key, UserOP::kRead);
      ...
      auto ol_record = std::make_shared<DataSetItem>(
          (table_id_t)TPCCTableType::kOrderLineTable,
          tpcc_order_line_val_t_size, order_line_key.item_key, UserOP::kUpdate);
      txn->AddToReadWriteSet(ol_record);
    ...
    cust_record->SetUpdate(tpcc_customer_val_bitmap::c_delivery_cnt,
                           &cust_val->c_delivery_cnt,
                           sizeof(cust_val->c_delivery_cnt));
...
bool TxOrderStatus(TPCC *tpcc_client, FastRandom *random_generator,
  ...
  */
  ...
  txn->Begin(tx_id, TXN_TYPE::kROTxn, "TxOrderStatus");
  ...
  const uint32_t district_id = tpcc_client->RandomNumber(
      random_generator[txn->coro_id], district_id_start, district_id_end_);
  uint32_t customer_id = 0;
  ...
  int32_t order_id =
      tpcc_client->RandomNumber(random_generator[txn->coro_id], 1,
                                tpcc_client->num_customer_per_district);
  uint64_t o_key =
  ...
  auto order_record = std::make_shared<DataSetItem>(
      (table_id_t)TPCCTableType::kOrderTable, tpcc_order_val_t_size,
      order_key.item_key, UserOP::kRead);
  ...
  tpcc_order_val_t *order_val = (tpcc_order_val_t *)order_record->Value();
  if (order_val->o_entry_d == 0) {
    RDMA_LOG(FATAL) << "[FATAL] Read order unmatch, tid-cid-txid: " << txn->t_id
                    << "-" << txn->coro_id << "-" << tx_id;
  }
  ...
  for (int i = 1; i <= order_val->o_ol_cnt; i++) {
    ...
    order_line_key.ol_id = ol_key;
    auto ol_record = std::make_shared<DataSetItem>(
        (table_id_t)TPCCTableType::kOrderLineTable, tpcc_order_line_val_t_size,
        order_line_key.item_key, UserOP::kRead);
...
bool TxStockLevel(TPCC *tpcc_client, FastRandom *random_generator,
  ...
  txn->Begin(tx_id, TXN_TYPE::kROTxn, "TxStockLevel");

  int32_t threshold =
      tpcc_client->RandomNumber(random_generator[txn->coro_id],
  ...
  auto dist_record = std::make_shared<DataSetItem>(
      (table_id_t)TPCCTableType::kDistrictTable, tpcc_district_val_t_size,
      dist_key.item_key, UserOP::kRead);
  ...
  for (int order_id = o_id - tpcc_stock_val_t::STOCK_LEVEL_ORDERS;
    ...
    for (int line_number = 1; line_number <= tpcc_order_line_val_t::MAX_OL_CNT;
      ...
      auto stock_record = std::make_shared<DataSetItem>(
          (table_id_t)TPCCTableType::kStockTable, tpcc_stock_val_t_size,
          stock_key.item_key, UserOP::kRead);
