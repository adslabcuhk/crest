...
class TPCC {
 public:
  ...
  int64_t MakeCustomerKey(int32_t w_id, int32_t d_id, int32_t c_id) {
    ...
    int64_t id = upper_id * num_customer_per_district + c_id;
  ...
  int64_t MakeNewOrderKey(int32_t w_id, int32_t d_id, int32_t o_id) {
    ...
    int32_t upper_id = (w_id - 1) * num_district_per_warehouse + (d_id - 1);
    int64_t id = upper_id * DISTRICT_MAX_OID + (o_id - 1);
    return id;
  }
  ...
  int64_t MakeOrderLineKey(int32_t w_id, int32_t d_id, int32_t o_id, int32_t number) {
    ...
    int64_t oid = static_cast<int64_t>(upper_id) * 20000 + static_cast<int64_t>(o_id);
    int64_t olid = oid * 15 + number - 1; 
