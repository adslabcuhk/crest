...
static int DISTRICT_MAX_OID = 20000;
...
enum tpcc_customer_index_val_bitmap : int { c_id = 0 };
...
constexpr size_t tpcc_customer_index_val_t_size =
    sizeof(tpcc_customer_index_val_t);

// static_assert(sizeof(tpcc_customer_index_val_t) == 16, "");  // add debug
// magic
...
enum tpcc_history_val_bitmap : int { h_amount = 0, h_date, h_data };
...
enum tpcc_new_order_val_bitmap : int { no_dummy = 0 };
...
enum tpcc_order_index_val_bitmap : int { o_id = 0 };
...
enum tpcc_item_val_bitmap : int { i_im_id = 0, i_price, i_name, i_data };
