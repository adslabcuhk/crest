...
void SmallBank::LoadTable(node_id_t node_id,
  ...
  {
    RDMA_LOG(DBG) << "Loading ACCOUNTS table";
    accounts_table = new HashStore((table_id_t)SmallBankTableType::kAccountsTable,
                                   table_config.get("num_accounts").get_uint64(),
                                   mem_store_alloc_param);
    PopulateAccountsTable();
    total_size += accounts_table->GetTotalSize();
    ht_loadfv_size += accounts_table->GetHTInitFVSize();
    ht_size += accounts_table->GetHTSize();
    initfv_size += accounts_table->GetInitFVSize();
    real_cvt_size += accounts_table->GetLoadCVTSize();
  }
  ...
  if ((node_id_t)SmallBankTableType::kAccountsTable % num_server == node_id) {
    RDMA_LOG(EMPH) << "[Primary] ACCOUNTS table ID: " << (node_id_t)SmallBankTableType::kAccountsTable;
    std::cerr << "Number of initial records: " << std::dec << accounts_table->GetInitInsertNum() << std::endl;
    primary_table_ptrs.push_back(accounts_table);
  }
  ...
  if (BACKUP_NUM < num_server) {
    for (node_id_t i = 1; i <= BACKUP_NUM; i++) {
      ...
      if ((node_id_t)SmallBankTableType::kAccountsTable % num_server == (node_id - i + num_server) % num_server) {
        RDMA_LOG(DBG) << "[Backup] CHECKING table ID: " << (node_id_t)SmallBankTableType::kAccountsTable;
        std::cerr << "Number of initial records: " << std::dec << accounts_table->GetInitInsertNum() << std::endl;
        backup_table_ptrs.push_back(accounts_table);
      }
    }
  }
}
...
void SmallBank::PopulateAccountsTable() {
  /* Populate the tables */
  for (uint32_t acct_id = 0; acct_id < num_accounts_global; acct_id++) {
    // Accounts
    smallbank_accounts_key_t accounts_key;
    accounts_key.acct_id = (uint64_t)acct_id;

    smallbank_accounts_val_t accounts_val;
    accounts_val.magic = smallbank_checking_magic;
    std::string n = "account-" + std::to_string(acct_id);
    strcpy(accounts_val.name, n.c_str());

    LoadRecord(accounts_table, accounts_key.item_key,
               (void*)&accounts_val, sizeof(smallbank_accounts_val_t),
