...
size_t TXN::CollectDeleteNewestAttr(AttrPos *attr_pos, bitmap_t read_pos_bmp,
                                    table_id_t table_id) {
  size_t must_read_attrs_len = 0;
  offset_t offset_in_struct = 0;

  uint64_t mask = 0x1;
  for (int attr_idx = 1; attr_idx <= ATTRIBUTE_NUM[table_id];
       attr_idx++, mask <<= 1) {
    offset_in_struct += ATTR_SIZE[table_id][attr_idx - 1];
    if (read_pos_bmp & mask) {
      // Read vcell[next_pos]'s modified attributes. These attributes must be
      // read
...
size_t TXN::CollectDeleteMiddleAttr(AttrPos *attr_pos, CVT *cvt, int read_pos,
                                    table_id_t table_id) {
  size_t must_read_attrs_len = 0;
  offset_t offset_in_struct = 0;
...
bool TXN::IsFurtherModified(uint64_t mask, CVT *cvt, int read_pos) {
  for (int vc_id = (read_pos + 1) % MAX_VCELL_NUM;
       (cvt->vcell[vc_id].valid) &&
       (cvt->vcell[vc_id].version > cvt->vcell[read_pos].version);
...
void TXN::CollectAttr(std::vector<AttrRead> &attr_read_list, AttrPos *attr_pos,
                      // use pointer to reduce copy into pending_value_read
                      std::vector<OldAttrPos> *old_attr_pos,
                      table_id_t table_id, CVT *cvt, int next_pos,
  ...
  for (int attr_idx = 1; attr_idx <= ATTRIBUTE_NUM[table_id];
    ...
    if (next_pos_bmp & mask) {
      ...
      SearchOldVCells(attr_idx, mask, attr_read_list, old_attr_pos, table_id,
                      cvt, next_pos);
    }
  }
  ...
  char *must_read_attrs_buf =
      thread_rdma_buffer_alloc->Alloc(must_read_attrs_len);
  attr_pos->local_attr_buf = must_read_attrs_buf;

  attr_read_list.emplace_back(
      AttrRead{.local_attr_buf = must_read_attrs_buf,
               .remote_attr_off = cvt->vcell[next_pos].attri_so +
                                  cvt->header.remote_attribute_offset,
               .attr_size = must_read_attrs_len});
}

void TXN::SearchOldVCells(int attr_idx, uint64_t mask,
                          std::vector<AttrRead> &attr_read_list,
                          std::vector<OldAttrPos> *old_attr_pos,
                          table_id_t table_id, CVT *cvt, int next_pos) {
  // We need to check which old vcell contains this modification, from old to
  // new. The vcell id should >=0. The vcell should be valid. The vcell's
  // version should > start_time to collect future-than-me undos
  for (int vc_id = (next_pos + 1) % MAX_VCELL_NUM;
    ...
    if (bitmap & mask) {
      // If this vcell contains the modified attribute, I need to calculate the
      // remote address of this attribute. To achieve this, I need to first
      // calculate the offset of this attribute in all the modified attributes
      // of this vcell This offset is called attr_inner_off Moreover, I also
      // need to calculate the offset this attribute in the DB record for future
      // apply&copy value. This offset is called off_within_struct
      ...
      //   (1) attr_inner_off is the total size of modified (i.e., `1`)
      //   attributes ranked before the attr_idx-th attribute (2)
      //   off_within_struct is the total size of all attributes ranked before
      //   the attr_idx-th attribute
      ...
      attr_read_list.emplace_back(AttrRead{
          .local_attr_buf = attr_buf,
          .remote_attr_off = cvt->vcell[vc_id].attri_so + attr_inner_off +
                             cvt->header.remote_attribute_offset,
          .attr_size = attr_sz});
