...
bool TXN::IssueReadROCVT(std::vector<DirectRead> &pending_direct_ro,
  ...
  for (int i = 0; i < read_only_set.size(); i++) {
    if (read_only_set[i]->is_fetched)
      continue;
    node_id_t remote_node_id = global_meta_man->GetPrimaryNodeIDWithCrash(
        read_only_set[i]->header.table_id);

#if HAVE_PRIMARY_CRASH
    if (remote_node_id == PRIMARY_CRASH) {
      // RDMA_LOG(INFO) << "Thread " << t_id << " primary fails at readrocvt.
      // table_id = " << read_only_set[i]->header.table_id << " txn id: " <<
      // tx_id;
      RecoverPrimary(read_only_set[i]->header.table_id);
      event_counter.RegEvent(t_id, txn_name,
                             "IssueReadROCVT:RecoverPrimary:Abort");
      return false;
    }
#endif
    ...
    if (remote_node_id == BACKUP_CRASH) {
      event_counter.RegEvent(t_id, txn_name,
                             "IssueReadROCVT:DiscoveryBackupFail");
      return false;
    }
#endif
    ...
    auto offset =
        addr_cache->Search(remote_node_id, read_only_set[i]->header.table_id,
                           read_only_set[i]->header.key);
    if (offset != NOT_FOUND) {
      ...
      HashMeta meta = global_meta_man->GetPrimaryHashMetaWithTableID(
          read_only_set[i]->header.table_id);
      uint64_t bkt_idx = GetHash(read_only_set[i]->header.key, meta.bucket_num,
                                 meta.hash_core);
      ...
      pending_hash_read.emplace_back(
          HashRead{.qp = qp,
                   .item = read_only_set[i].get(),
                   ...
                   .is_ro = true});
      coro_sched->RDMARead(coro_id, qp, local_hash_bucket, bucket_off,
                           bkt_size);
...
bool TXN::IssueReadLockCVT(std::vector<CasRead> &pending_cas_rw,
  ...
  for (int i = 0; i < read_write_set.size(); i++) {
    ...
    auto remote_node_id = global_meta_man->GetPrimaryNodeIDWithCrash(
        read_write_set[i]->header.table_id);
#if HAVE_PRIMARY_CRASH
    if (remote_node_id == PRIMARY_CRASH) {
      // RDMA_LOG(INFO) << "Thread " << t_id << " primary fails at rlockcvt.
      // table_id = " << read_write_set[i]->header.table_id << " txn id: " <<
      // tx_id;
      RecoverPrimary(read_write_set[i]->header.table_id);
      event_counter.RegEvent(t_id, txn_name,
                             "IssueReadLockCVT:RecoverPrimary:Abort");
      return false;
    }
#endif
    ...
    if (remote_node_id == BACKUP_CRASH) {
      event_counter.RegEvent(t_id, txn_name,
                             "IssueReadLockCVT:DiscoveryBackupFail");
      return false;
    }
#endif
    ...
    RCQP *qp = thread_qp_man->GetRemoteDataQPWithNodeID(remote_node_id);
    auto offset =
        addr_cache->Search(remote_node_id, read_write_set[i]->header.table_id,
                           read_write_set[i]->header.key);
    ...
    if (offset != NOT_FOUND) {
      ...
      // (kqh): This is how we detect the conflicts between NewOrder transaction
      ...
      // (kqh): We need to embedd the thread_id into the lock_value: 
      lock_value |= ((uint64_t)t_id << 32);
      doorbell->SetLockReq(cas_buf, read_write_set[i]->GetRemoteLockAddr(),
                           STATE_UNLOCKED, lock_value);
      doorbell->SetReadReq(cvt_buf, offset, CVTSize);
      doorbell->SendReqs(coro_sched, qp, coro_id);

      // CheckAddr(read_write_set[i]->GetRemoteLockAddr(), 8,
      // "IssueReadLockCVT:SetLockReq"); CheckAddr(offset, CVTSize,
      // "IssueReadLockCVT:SetReadReq");
      ...
      bool need_read = true;
      // Only read
      const HashMeta &meta = global_meta_man->GetPrimaryHashMetaWithTableID(
          read_write_set[i]->header.table_id);
      uint64_t bkt_idx = GetHash(read_write_set[i]->header.key, meta.bucket_num,
                                 meta.hash_core);
      offset_t bucket_off = bkt_idx * meta.bucket_size + meta.base_off;
      ...
      if (read_write_set[i]->user_op == UserOP::kInsert) {
        // (kqh): We can check if the transaction has already read the hash
        // bucket before
        for (const auto &t : pending_insert_off_rw) {
          if (t.remote_node == remote_node_id && t.bucket_off == bucket_off) {
            need_read = false;
            local_hash_bucket = t.buf;
            break;
          }
        }
        ...
        pending_hash_read.emplace_back(HashRead{.qp = qp,
                                                .item = read_write_set[i].get(),
                                                .buf = local_hash_bucket,
                                                .remote_node = remote_node_id,
                                                .item_idx = i,
