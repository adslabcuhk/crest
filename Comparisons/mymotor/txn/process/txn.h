...
class TXN {
  // If this is the new order transaction, it supposed to change the lock value
  // of the rw record to be 2
  static constexpr uint64_t NEWORDER_LOCK_VALUE = 0xabcd;
  static constexpr uint64_t PAYMENT_LOCK_VALUE = 0xef12;
...
public:
  ...
  //   if ((start < 0) ||
  //       (start + len > (global_meta_man->delta_start_off +
  //       global_meta_man->per_thread_delta_size * MAX_CLIENT_NUM_PER_MN))) {
  //     TLOG(ERROR, t_id) << "Error remote access addr. start: " << start << ".
  //     start+len: " << start + len << ". desc: " << desc;
  //   }
  // }

  /*****************************************************/

  int GetTxnExecTime() { return exec_time; }

  int GetValidateAndLogTime() { return validate_and_log_time; }

  int GetCommitTime() { return commit_dura; }
...
public:
  TXN(MetaManager *meta_man, QPManager *qp_man, t_id_t tid, coro_id_t coroid,
      CoroutineScheduler *sched, LocalBufferAllocator *rdma_buffer_allocator,
      RemoteDeltaOffsetAllocator *delta_offset_allocator,
      LockedKeyTable *locked_key_table, AddrCache *addr_buf) {
    // Transaction setup
    tx_id = 0;
    t_id = tid;
    coro_id = coroid;
...
private:
  ...
  void WriteReplica(RCQP *qp, const DataSetItem *item, int write_pos,
                    uint8_t user_op, bool new_attr_bar);

  void HandleDelete(RCQP *qp, const DataSetItem *item, int write_pos);

  void HandleUpdate(RCQP *qp, const DataSetItem *item, int write_pos,
                    bool new_attr_bar);

  void HandleInsert(RCQP *qp, const DataSetItem *item, int write_pos);

  void Abort();

  void RecoverPrimary(table_id_t table_id, PrimaryCrashTime p_crash_time =
                                               PrimaryCrashTime::kBeforeCommit);

  void RecoverBackup(table_id_t table_id, node_id_t to_recover_backup_node_id);

  void SendMsgToReplica(node_id_t copy_from, node_id_t copy_to,
                        table_id_t table_id, int is_primary_fail);

  int FindReadPos(CVT *cvt, bool &is_read_newest, int &max_pos, bool &is_ea,
                  bool &is_all_invalid);
  ...
  int ReCheckReadPosForDelete(CVT *cvt, uint64_t current_time,
                              bool &is_all_invalid);

  int FindCasReadPos(CVT *cvt, int &read_pos, bool &is_read_newest,
                     int &max_pos, bool &is_ea);
...
private:
  ...
  bool ReadValueRO(RCQP *qp, CVT *fetched_cvt, DataSetItem *item_ptr,
                   int read_pos, std::vector<ValueRead> &pending_value_read,
                   bool is_read_newest);

  bool ReadValueRW(RCQP *qp, CVT *fetched_cvt, DataSetItem *item_ptr,
                   int read_pos, std::vector<ValueRead> &pending_value_read,
                   bool is_read_newest);

  void CollectAttr(std::vector<AttrRead> &attr_read_list, AttrPos *attr_pos,
                   std::vector<OldAttrPos> *old_attr_pos, table_id_t table_id,
                   CVT *cvt, int read_pos, DataSetItem *item_ptr);

  void SearchOldVCells(int attr_idx, uint64_t mask,
                       std::vector<AttrRead> &attr_read_list,
                       std::vector<OldAttrPos> *old_attr_pos,
                       table_id_t table_id, CVT *cvt, int read_pos);

  size_t CollectDeleteNewestAttr(AttrPos *attr_pos, bitmap_t read_pos_bmp,
                                 table_id_t table_id);

  size_t CollectDeleteMiddleAttr(AttrPos *attr_pos, CVT *cvt, int read_pos,
                                 table_id_t table_id);
  ...
  void CopyValueAndAttr(DataSetItem *item, char *fetched_value,
                        AttrPos *attr_pos,
                        std::vector<OldAttrPos> *old_attr_pos,
                        size_t value_size);
  ...
  int FindMatch(HashRead &res, int &read_pos, bool &is_read_newest);

  bool CheckInsertCVT(std::vector<InsertOffRead> &pending_insert_off_rw,
                      std::vector<LockReadCVT> &pending_cvt_insert,
                      std::vector<ValueRead> &pending_value_read);

  int FindInsertOff(InsertOffRead &res, int &read_pos, bool &is_read_newest);

  bool LockReadValueRW(RCQP *qp, node_id_t remote_node, CVT *fetched_cvt,
                       DataSetItem *item_ptr, int read_pos,
                       std::vector<ValueRead> &pending_value_read, int item_idx,
                       bool is_read_newest);
...
private:
  ...
  struct pair_hash {
    inline std::size_t
    operator()(const std::pair<node_id_t, offset_t> &v) const {
  ...
  // (kqh) Internal latency breakdown statistics:
  uint64_t exec_time = 0;             // The time of acquiring lock
  uint64_t validate_and_log_time = 0; // The time of doing validation
  uint64_t commit_dura = 0;           // The time spent on applying writes
};
...
#if OUTPUT_KEY_STAT
  key_counter.RegKey(t_id, KeyType::kKeyRead, txn_name, item->header.table_id,
                     item->header.key);
#endif
...
#if OUTPUT_KEY_STAT
  key_counter.RegKey(t_id, KeyType::kKeyWrite, txn_name, item->header.table_id,
                     item->header.key);
#endif
...
ALWAYS_INLINE
int TXN::FindReadPos(CVT *cvt, bool &is_read_newest, int &max_pos, bool &is_ea,
                     bool &is_all_invalid) {
  ...
  for (int i = 0; i < MAX_VCELL_NUM; i++) {
    ...
    // (kqh): Update tx_id_generator
    uint64_t id = tx_id_generator.load();
    while (id < ts) {
      tx_id_generator.compare_exchange_strong(id, ts);
      id = tx_id_generator.load();
    }

    if (ts > start_time) {
      // is_read_newest = false;
      start_time = std::max(start_time, (uint64_t)ts);

      // (kqh): If there is a ts > start_time, we always read the latest version
      max_pos = i;
      max_ts = ts;
      target_ts = ts;
      target_idx = i;
      ...
      if ((global_meta_man->iso_level == ISOLATION::SR) &&
        ...
        table_id_t table_id = cvt->header.table_id;
        // In SR and rw txn, if reading a version larger than start time, I can
        // early abort, since in Validation I must abort by acquiring a more
        // larger Tcommit
        std::string event_name =
            "FindReadPos:EarlyAbort, Table" + std::to_string(table_id);
...
// ALWAYS_INLINE
// int TXN::ReGetInsertPos(CVT* cvt, uint64_t current_time, bool&
// is_all_invalid) {
...
ALWAYS_INLINE
int TXN::ReCheckReadPosForDelete(CVT *cvt, uint64_t current_time,
                                 bool &is_all_invalid) {
  int target_idx = NO_POS;
...
ALWAYS_INLINE
int TXN::FindCasReadPos(CVT *cvt, int &read_pos, bool &is_read_newest,
                        int &max_pos, bool &is_ea) {
  read_pos = NO_POS;
  ...
  for (int i = 0; i < MAX_VCELL_NUM; i++) {
    ...
    if (cvt->vcell[i].valid) {
      // (kqh): Update tx_id_generator to be the maximum version it has
      // encountered This ensures that the commit timestamp of this transaction
      // > any version it has encounterred
      uint64_t id = tx_id_generator.load();
      while (id < ts) {
        tx_id_generator.compare_exchange_strong(id, ts);
        id = tx_id_generator.load();
    ...
    if (ts > start_time && cvt->vcell[i].valid) {
      // is_read_newest = false;
      // (kqh): if there is a newer version, we always read the latest version
      is_read_newest = true;
      read_pos = i;
      max_pos = i;
      max_ts = ts;
      ...
      if ((global_meta_man->iso_level == ISOLATION::SR) &&
          (txn_type == TXN_TYPE::kRWTxn)) {
        // In SR and rw txn, if reading a version larger than start time, I can
        // early abort, since in Validation I must abort due to acquiring a
        // larger Tcommit
        is_ea = true;
        // access_old_version_cnt[t_id]++;
        return NO_POS;
...
ALWAYS_INLINE
int TXN::FindReadWritePos(CVT *cvt, int &new_read_pos, int &max_pos,
                          bool &is_ea) {
  new_read_pos = NO_POS;
...
ALWAYS_INLINE
bool TXN::RDMAWriteRoundTrip(RCQP *qp, char *wt_data, uint64_t remote_offset,
                             size_t size) {
...
// rd_data
ALWAYS_INLINE
bool TXN::RDMAReadRoundTrip(RCQP *qp, char *rd_data, uint64_t remote_offset,
                            size_t size) {
