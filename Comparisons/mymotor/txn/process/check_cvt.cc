...
bool TXN::CheckDirectROCVT(std::vector<DirectRead> &pending_direct_ro,
  ...
  for (auto &res : pending_direct_ro) {
    ...
    if (likely(fetched_cvt->header.key == local_item->header.key &&
      ...
      int read_pos = FindReadPos(fetched_cvt, is_read_newest, max_version_pos,
                                 is_ea, is_all_invalid);

      if (is_all_invalid) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckDirectROCVT:FindReadPos:AllInvalid");
        return false;
      }
      ...
      if (is_ea) {
        event_counter.RegEvent(
            t_id, txn_name,
            "CheckDirectROCVT:FindReadPos:NoReadPos:EarlyAbort");
        return false;
      }
      ...
      if (read_pos == NO_POS) {
        event_counter.RegEvent(
            t_id, txn_name,
            "CheckDirectROCVT:FindReadPos:NoReadPos (could due to try read)");
        return false;
      }
      ...
      if (fetched_cvt->vcell[read_pos].IsWritten()) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckDirectROCVT:VcellIsWritten");
        return false;
      }

      local_item->vcell = fetched_cvt->vcell[read_pos];

      if (!ReadValueRO(res.qp, fetched_cvt, res.item, read_pos,
                       pending_value_read, is_read_newest)) {
        return false;
      }

    } else {
      // The cached address is stale, i.e., key1 is deleted and the slot is
      // occupied by another key2 You need a hash read next time to get the
      // correct new slot of the key1
      addr_cache->Insert(res.remote_node, local_item->header.table_id,
                         local_item->header.key, NOT_FOUND);
      event_counter.RegEvent(t_id, txn_name,
                             "CheckDirectROCVT:CachedAddrStale");
      return false;
    }
  }
  return true;
...
bool TXN::CheckCasReadCVT(std::vector<CasRead> &pending_cas_rw,
  ...
  for (auto &res : pending_cas_rw) {
    if (*((lock_t *)res.cas_buf) == STATE_UNLOCKED) {
      locked_rw_set.emplace_back(res.idx);
    } else {
      std::string event_name =
          "CheckCasReadCVT:LockFail:" + std::string("Table") +
          std::to_string(res.item->header.table_id);
  ...
  for (auto &res : pending_cas_rw) {
    if (*((lock_t *)res.cas_buf) != STATE_UNLOCKED) {
      {
        // This is how we deal with NewOrder and Payment transaction conflicts
        lock_t lock_value = *(lock_t *)(res.cas_buf);
        // txn_specific_lock_value is the lower 32bits
        uint64_t txn_specific_lock_value = lock_value & 0xFFFFFFFFULL;
        // thread_id is the higher 32bits without the invisible bit
        uint64_t locking_txn_thread_id = (lock_value) >> 32;
        // printf("Lock value = %lu\n", lock_value);

        // The case that two types of transactions contend on the different
        // column of the same record
        if (txn_specific_lock_value == NEWORDER_LOCK_VALUE &&
            txn_name == "TxPayment") {
          std::string event_name = "CheckCasReadCVT:LockFail:"
        ...
        // The case that two types of transactions contend on the same column of
        // the same record, and they are running on the same CN
        if (locking_txn_thread_id / THREAD_PER_CN == t_id / THREAD_PER_CN) {
          ...
          if (conflicting_lock) {
            ...
            std::string event_name = "CheckCasRW::LockFail::SameCN::Table" +
                                     std::to_string(res.item->header.table_id);
            event_counter.RegEvent(t_id, txn_name, event_name);
          }
        }

        // The general case that lock is acquired
        std::string event_name = "CheckCasReadCVT::Lock is acquired::Table" +
                                 std::to_string(res.item->header.table_id);
        event_counter.RegEvent(t_id, txn_name, event_name);
      }
    ...
    DataSetItem *local_item = res.item;
    // (kqh): For debug, we want to see why the Value and VCell mismatch
    local_item->local_cvt = fetched_cvt;
    ...
    if (likely(fetched_cvt->header.key == local_item->header.key &&
      ...
      if (local_item->user_op == UserOP::kDelete) {
        read_pos = FindReadPos(fetched_cvt, is_read_newest, max_version_pos,
                               is_ea, is_all_invalid);
        ...
        if (is_ea) {
          event_counter.RegEvent(
              t_id, txn_name,
              "CheckCasReadCVT:Delete:FindReadPos:NoReadPos:EarlyAbort");
          return false;
        }

        if (read_pos == NO_POS) {
          event_counter.RegEvent(
              t_id, txn_name, "CheckCasReadCVT:Delete:FindReadPos:NoReadPos");
          return false;
        }
        ...
        // Update
        write_pos = FindCasReadPos(fetched_cvt, read_pos, is_read_newest,
                                   max_version_pos, is_ea);

        if (is_ea) {
          event_counter.RegEvent(t_id, txn_name,
                                 "CheckCasReadCVT:FindCasReadPos:EarlyAbort");
          return false;
        }

        if (read_pos == NO_POS) {
          if (local_item->user_op == UserOP::kInsert &&
              IsAllInvalid(fetched_cvt)) {
            local_item->header = fetched_cvt->header;
            local_item->target_write_pos = 0;
            continue;
          } else {
            event_counter.RegEvent(t_id, txn_name,
                                   "CheckCasReadCVT:FindCasReadPos:NoReadPos");
            return false;
          }
        }
        ...
        if (write_pos == NO_POS) {
          event_counter.RegEvent(t_id, txn_name,
                                 "CheckCasReadCVT:FindCasReadPos:NoWritePos");
          return false;
        }
        ...
        local_item->remote_bmp =
            fetched_cvt->vcell[max_version_pos].attri_bitmap;
      }

      if (fetched_cvt->vcell[read_pos].IsWritten()) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckCasReadCVT:VcellIsWritten");
        return false;
      }
      ...
      if (!ReadValueRW(res.qp, fetched_cvt, res.item, read_pos,
                       pending_value_read, is_read_newest)) {
        return false;
      }
    } else {
      ...
      // occupied by another key2 You need a hash read next time to get the
      // correct new slot of the key1
      addr_cache->Insert(res.primary_node_id, local_item->header.table_id,
                         local_item->header.key, NOT_FOUND);
...
bool TXN::CheckHashReadCVT(std::vector<HashRead> &pending_hash_read,
  ...
  for (auto &res : pending_hash_read) {
    ...
    res.item->local_cvt = fetched_cvt;
    ...
    if (res.is_ro) {
      ...
      if (!LockReadValueRW(res.qp, res.remote_node, fetched_cvt, res.item,
                           read_pos, pending_value_read, res.item_idx,
                           is_read_newest)) {
        return false;
      }
    }
...
int TXN::FindMatch(HashRead &res, int &read_pos, bool &is_read_newest) {
  // auto* fetched_hash_bucket = (HashBucket*)res.buf;
  DataSetItem *local_item = res.item;

  for (int slot_idx = 0; slot_idx < SLOT_NUM[local_item->header.table_id];
       slot_idx++) {
    ...
    if (fetched_cvt->header.value_size > 0) {
      addr_cache->Insert(res.remote_node, fetched_cvt->header.table_id,
    ...
    if (fetched_cvt->header.key == local_item->header.key &&
      ...
      if (is_ea) {
        event_counter.RegEvent(
            t_id, txn_name, "HashFindMatch:FindReadPos:NoReadPos:EarlyAbort");
        return NOT_FOUND;
      }
      ...
      if (read_pos == NO_POS) {
        if (res.item->user_op == UserOP::kDelete) {
          event_counter.RegEvent(t_id, txn_name,
                                 "HashFindMatch:Delete:FindReadPos:NoReadPos");
        } else {
          event_counter.RegEvent(t_id, txn_name,
                                 "HashFindMatch:Update:FindReadPos:NoReadPos");
        }
  ...
  event_counter.RegEvent(t_id, txn_name,
                         "HashFindMatch:NoMatch (Could due to try read)");
  return NOT_FOUND;
}
  ...
  for (auto &res : pending_insert_off_rw) {
    ...
    if (res.item->user_op == UserOP::kInsert) {
      ...
      *(lock_t *)lock_buff = 0xdeadbeaf;
      ...
      doorbell->SetLockReq(lock_buff, res.item->GetRemoteLockAddr(),
      ...
      // CheckAddr(res.item->GetRemoteLockAddr(), 8,
      // "CheckInsertCVT:SetLockReq"); CheckAddr(res.item->header.remote_offset,
      // CVTSize, "CheckInsertCVT:SetReadReq");
...
int TXN::FindInsertOff(InsertOffRead &res, int &read_pos,
                       bool &is_read_newest) {
  ...
  for (int i = 0; i < SLOT_NUM[local_item->header.table_id]; i++) {
    ...
    // Here we do not need to judge whether the empty cvt is locked or not,
    // since the locking status can be detected in Validation phase
    if ((insert_cvt_pos == NOT_FOUND) &&
        (fetched_cvt->header.value_size == 0)) {
      // Within a txn, multiple items cannot insert into the same slot
      std::pair<node_id_t, offset_t> new_pos(res.remote_node,
                                             res.bucket_off + i * CVTSize);
      ...
      if (is_ea) {
        event_counter.RegEvent(t_id, txn_name,
                               "FindInsertOff:FindReadPos:EarlyAbort");
        return NOT_FOUND;
      }

      if (read_pos == NO_POS) {
        event_counter.RegEvent(t_id, txn_name,
                               "FindInsertOff:FindReadPos:NoReadPos");
        return NOT_FOUND;
      }
  ...
  if (real_insert) {
    if (insert_cvt_pos == NOT_FOUND) {
      std::string table_name =
          ":Table" + std::to_string(local_item->header.table_id);
      event_counter.RegEvent(t_id, txn_name,
                             "FindInsertOff:NoEmptySlot" + table_name);
      return NOT_FOUND;
    }
    // If it is an insert, we do not assign fetched_cvt's header and vcell to
    // local item Because fetched_cvt could be empty without a useful header and
    // vcell
