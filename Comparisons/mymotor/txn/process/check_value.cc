...
// --------------- Checking cvts, values and attributes after reading
// them---------
  ...
  for (auto &fetched_it : pending_value_read) {
    ...
    if (fetched_value_sa != fetched_value_ea) {
      event_counter.RegEvent(t_id, txn_name,
                             "CheckValueRO:ValueAnchorMismatch");
      return false;
    }

    // For read-only txns, they do not validate, so they need to check the
    // consistency between the anchors in vcell and vpkg
    if (fetched_value_ea != fetched_it.item->latest_anchor) {
      event_counter.RegEvent(t_id, txn_name,
                             "CheckValueRO:ValueVcellAnchorMismatch");
      return false;
    }
    ...
    case Content::kValue: {
      ...
      memcpy((char *)&(fetched_it.item->valuepkg.value), fetched_value,
             value_size);
      break;
    }
    case Content::kValue_Attr: {
      // Case 2: read values and attributes
      CopyValueAndAttr(fetched_it.item, fetched_value, fetched_it.attr_pos,
                       fetched_it.old_attr_pos, value_size);
      break;
    }
    default: {
...
bool TXN::CheckValueRW(std::vector<ValueRead> &pending_value_read,
  ...
  for (auto &fetched_it : pending_value_read) {
    if (fetched_it.cont == Content::kDelete_Vcell_LockCVT) {
      if (*((lock_t *)fetched_it.lock_buf) == STATE_UNLOCKED) {
        locked_rw_set.emplace_back(fetched_it.idx);
      }
    }
    ...
    if (fetched_it.cont == Content::kValue_LockCVT) {
      if (*((lock_t *)fetched_it.lock_buf) == STATE_UNLOCKED) {
        locked_rw_set.emplace_back(fetched_it.idx);
      }
    }

    if (fetched_it.cont == Content::kValue_Attr_LockCVT) {
      if (*((lock_t *)fetched_it.lock_buf) == STATE_UNLOCKED) {
        locked_rw_set.emplace_back(fetched_it.idx);
      }
    }
  ...
  for (auto &fetched_it : pending_value_read) {
    ...
    if (fetched_it.cont == Content::kDelete_Vcell_LockCVT) {
      if (*((lock_t *)fetched_it.lock_buf) != STATE_UNLOCKED) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckValueRW:kDelete_Vcell_LockCVT:CVTLocked");
        return false;
      }
    ...
    if (fetched_it.cont == Content::kDelete_AllInvalid_LockCVT) {
      ...
      if (*((lock_t *)fetched_it.lock_buf) != STATE_UNLOCKED) {
        event_counter.RegEvent(
            t_id, txn_name,
            "CheckValueRW:kDelete_AllInvalid_LockCVT:CVTLocked");
        return false;
      }

      bool is_all_invalid = true;

      auto new_read_pos = ReCheckReadPosForDelete((CVT *)(fetched_it.cvt_buf),
                                                  start_time, is_all_invalid);
      ...
      if (is_all_invalid) {
        ...
        event_counter.RegEvent(
            t_id, txn_name,
            "CheckValueRW:kDelete_AllInvalid_LockCVT:NewVersionOccurs");
        return false;
      }
    }
    ...
    if (fetched_value_sa != fetched_value_ea) {
      event_counter.RegEvent(t_id, txn_name,
                             "CheckValueRW:ValueAnchorMismatch");
      return false;
    }
    ...
    if (fetched_value_ea != fetched_it.item->latest_anchor) {
      std::string table_name =
          ":Table" + std::to_string(fetched_it.item->header.table_id);
      event_counter.RegEvent(
          t_id, txn_name, "CheckValueRW:ValueVcellAnchorMismatch" + table_name);
      // TLOG(INFO, t_id) << "fetched_value_ea: " << (int)fetched_value_ea << "
      // latest_anchor: " << (int)fetched_it.item->latest_anchor << " table id:
      // " << fetched_it.item->header.table_id << " key: " <<
      // fetched_it.item->header.key;
      // auto t = time(nullptr);
#if WORKLOAD_TPCC
      // For TPC-C, we do not return false here
...
#elif WORKLOAD_MICRO || WORKLOAD_SMALLBANK
      // srand(time(nullptr));
      // auto t = rand();
      // if (t % 100 < 80) {
      //   return false;
    ...
    case Content::kValue_LockCVT: {
      ...
      if (*((lock_t *)fetched_it.lock_buf) != STATE_UNLOCKED) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckValueRW:kValue_LockCVT:CVTLocked");
        return false;
      }
    ...
    case Content::kValue_Attr_LockCVT: {
      ...
      if (*((lock_t *)fetched_it.lock_buf) != STATE_UNLOCKED) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckValueRW:kValue_Attr_LockCVT:CVTLocked");
        return false;
      }
    ...
    case Content::kDelete_Value_Attr_LockCVT: {
      ...
      if (*((lock_t *)fetched_it.lock_buf) != STATE_UNLOCKED) {
        event_counter.RegEvent(
            t_id, txn_name,
            "CheckValueRW:kDelete_Value_Attr_LockCVT:CVTLocked");
        return false;
      }
  ...
  for (auto &fetched_it : pending_cvt_insert) {
    // For insertions, the coordinator does not need to read values in Execution
    // phase
    if (*((lock_t *)fetched_it.lock_buf) != STATE_UNLOCKED) {
    ...
    if (fetched_it.item->is_insert_all_invalid) {
      ...
      // } else if (pos == NO_POS) {
      //   event_counter.RegEvent(t_id, txn_name,
      //   "CheckValueRW:Insert:ReGetInsertPosFail"); return false;
      // } else if (pos != NO_POS) {
      ...
      if (!IsAllInvalid(re_read_cvt)) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckValueRW:Insert:SlotBecomeValid");
        return false;
      }
    } else {
      if (re_read_cvt->header.value_size) {
        // I and another coordinator (C0) follow the same agreement to occupy
        // the first empty slot. Now I pass the locking check but I find that
        // the re-read cvt has been occupied. That's to say, C0 has finished
        // lock+write+unlock before I successfuly lock the remote slot. Due to
        // the agreement, I have to abort since I cannot overwrite C0's slot.
        // This is a rare case in practice though.
        event_counter.RegEvent(t_id, txn_name,
                               "CheckValueRW:Insert:SlotOccupied");
        return false;
      }
    }
...
void TXN::CopyValueAndAttr(DataSetItem *item, char *fetched_value,
                           AttrPos *attr_pos,
  ...
  if (old_attr_pos) {
    for (size_t i = 0; i < old_attr_pos->size(); i++) {
      memcpy(fetched_value + old_attr_pos->at(i).off_within_struct,
             old_attr_pos->at(i).local_attr_buf, old_attr_pos->at(i).len);
    }
  }

  char *p = attr_pos->local_attr_buf;
  for (size_t i = 0; i < attr_pos->offs_within_struct.size(); i++) {
    memcpy(fetched_value + attr_pos->offs_within_struct[i], p,
           attr_pos->lens[i]);
    p += attr_pos->lens[i];
  }
  ...
  if (global_meta_man->iso_level == ISOLATION::SR) {
    ...
    if (item->user_op == UserOP::kDelete) {
      ...
      if (new_read_pos == NO_POS) {
        event_counter.RegEvent(
            t_id, txn_name,
            "CheckValueRW:ObtainWritePos:ReReadPos:NoNewReadPosForDelete");
        return false;
      }
    } else {
      bool is_ea = false;
      write_pos =
          FindReadWritePos(re_read_cvt, new_read_pos, max_version_pos, is_ea);

      if (is_ea) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckValueRW:ObtainWritePos:EarlyAbort");
        return false;
      }
      ...
      if (write_pos == NO_POS) {
        event_counter.RegEvent(
            t_id, txn_name,
            "CheckValueRW:ObtainWritePos:FindReadWritePos:NoWritePosForUpdate");
        return false;
      }
      if (new_read_pos == NO_POS) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckValueRW:ObtainWritePos:FindReadWritePos:"
                               "NoNewReadPosForUpdate");
        return false;
      }
    ...
    if (re_read_cvt->vcell[new_read_pos].version != my_old_version) {
      ...
      event_counter.RegEvent(t_id, txn_name,
                             "CheckValueRW:ObtainWritePos:NewVersionOccurs");
      return false;
    }
  } else {
    // Below are for SI
    if (item->user_op != UserOP::kDelete) {
      ...
      if (write_pos == NO_POS) {
        event_counter.RegEvent(
            t_id, txn_name,
            "[SI] CheckValueRW:ObtainWritePos:FindWritePos:NoWritePos");
        return false;
      }
