...
MetaManager::MetaManager() {
    ...
    auto remote_meta_ports =
        mem_nodes.get("remote_meta_ports");  // Array Used for transferring datastore metas
    ...
    for (size_t index = 0; index < remote_ips.size(); index++) {
        ...
        if (remote_machine_id == -1) {
            RDMA_LOG(FATAL) << "Thread " << std::this_thread::get_id()
                            << " GetMemStoreMeta() failed!, remote_machine_id = -1";
        }
        int remote_port = (int)remote_ports.get(index).get_int64();
        remote_nodes.push_back(RemoteNode{.node_id = remote_machine_id,
                                          .ip = remote_ip,
                                          .port = remote_port,
                                          .meta_port = remote_meta_port});
    }
    ...
    for (auto p_meta : primary_hash_metas) {
        ...
        std::cerr << "Primary hash meta for TableID: " << p_meta.first << " HashMeta: "
                  << "<<<table_id: " << meta.table_id << ", table_ptr: 0x" << std::hex
                  << meta.table_ptr << ", base_off: 0x" << meta.base_off
                  << ", bucket_num: " << std::dec << meta.bucket_num
                  << ", bucket_size: " << meta.bucket_size << ", hash_core: " << (int)meta.hash_core
                  << ">>>" << std::endl;
    }
    ...
    for (size_t i = 0; i < primary_table_nodes.size(); i++) {
        ...
        for (auto meta : backup_hash_metas[i]) {
            std::cerr << "  HashMeta: <<<table_id: " << meta.table_id << ", table_ptr: 0x"
                      << std::hex << meta.table_ptr << ", base_off: 0x" << meta.base_off
                      << ", bucket_num: " << std::dec << meta.bucket_num
                      << ", bucket_size: " << meta.bucket_size
                      << ", hash_core: " << (int)meta.hash_core << " >>>" << std::endl;
        }
    }
    std::cerr << "---------------------------------------------------------------------------------"
                 "---------\n";
...
node_id_t MetaManager::GetMemStoreMeta(std::string& remote_ip, int remote_port) {
    ...
    if (remote_machine_id >= MAX_REMOTE_NODE_NUM) {
        RDMA_LOG(ERROR) << "remote machine id " << remote_machine_id
    ...
    RDMA_LOG(DBG) << "META MAN: delta_start_off (DataRegion size, MB): "
                  << (double)delta_start_off / 1024 / 1024 << ", per_thread_delta_size (MB): "
                  << (double)per_thread_delta_size / 1024 / 1024;

    // Get the `end of file' indicator: finish transmitting
    char* eof = snooper + sizeof(HashMeta) * (primary_meta_num + backup_meta_num);

    // Check meta
    // std::cerr << "--------------- Check meta of remote_ip: " << remote_ip << " remote_machine_id:
    // " << remote_machine_id << std::endl;
    if ((*((uint64_t*)eof)) == MEM_STORE_META_END) {
        ...
        for (size_t i = 0; i < primary_meta_num; i++) {
            ...
            // std::cerr << "tableID: " << meta.table_id << ", primary-nodeID: " <<
            // remote_machine_id << std::endl; std::cerr << "PrimaryHashMeta: table_id: " <<
            // meta.table_id << ", table_ptr: 0x" << std::hex << meta.table_ptr << ", base_off: 0x"
            // << meta.base_off << ", bucket_num: " << std::dec << meta.bucket_num << ",
            // bucket_size: " << meta.bucket_size << ", hash_core: " << (int)meta.hash_core << "
            // >>>" << std::endl;
        }
        ...
        for (size_t i = 0; i < backup_meta_num; i++) {
            ...
            // std::cerr << "tableID: " << meta.table_id << ", backup-nodeID: " << remote_machine_id
            // << std::endl; std::cerr << "BackupHashMeta: table_id: " << meta.table_id << ",
            // table_ptr: 0x" << std::hex << meta.table_ptr << ", base_off: 0x" << meta.base_off <<
            // ", bucket_num: " << std::dec << meta.bucket_num << ", bucket_size: " <<
            // meta.bucket_size << ", hash_core: " << (int)meta.hash_core << " >>>" << std::endl;
        }

    } else {
        RDMA_LOG(ERROR) << "EOF mismatches. Received: " << std::hex << "0x" << *((uint64_t*)eof)
                        << ", desired: " << std::hex << "0x" << MEM_STORE_META_END;
        free(recv_buf);
        abort();
    }
