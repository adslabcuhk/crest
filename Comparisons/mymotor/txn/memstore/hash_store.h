...
struct HashMeta {
  ...
  HashMeta(table_id_t table_id, uint64_t table_ptr, offset_t base_off,
           uint64_t bucket_n, size_t bucket_size, HashCore core_func)
      : table_id(table_id), table_ptr(table_ptr), base_off(base_off),
        bucket_num(bucket_n), bucket_size(bucket_size), hash_core(core_func) {}
  HashMeta() {}
} Aligned8;
...
class HashStore {
public:
  HashStore(table_id_t table_id, uint64_t bucket_n, MemStoreAllocParam *param,
            HashCore func = HashCore::kDirectFunc)
      : table_id(table_id), base_off(0), bucket_num(bucket_n),
        table_ptr(nullptr), value_ptr(nullptr),
        region_start_ptr(param->mem_region_start), hash_core(func),
        init_insert_num(0) {
    assert(bucket_num > 0);
    ...
    // (kqh): Always roundup total size to be 8 to make sure the next table
    // starts at 8-byte aligned address
    total_size = total_size + (total_size % 8 == 0 ? 0 : 8 - total_size % 8);

    if ((uint64_t)param->hash_store_start + param->alloc_offset + total_size >=
    ...
    // std::cerr << "Value size (B): " << TABLE_VALUE_SIZE[table_id] <<
    // std::endl; std::cerr << "Num of vcells: " << MAX_VCELL_NUM << std::endl;
    // // TODO: different tables have differnet # of vcells std::cerr << "CVT
    // size (B): " << CVTSize << std::endl; std::cerr << "Slot num: " <<
    // SLOT_NUM[table_id] << std::endl; std::cerr << "HashBucketNum: " <<
    // bucket_num << std::endl; std::cerr << "Bucket size (B): " << bkt_size <<
    // std::endl; std::cerr << "HashCore: " << (int)func << std::endl; std::cerr
    // << "HT+Vpkg size (MB): " << total_size / 1024 / 1024 << std::endl;
    // std::cerr << "Start address: " << std::hex << "0x" << (uint64_t)table_ptr
    // << std::endl; std::cerr << "Base_off:" << std::hex << "0x" << base_off <<
    // std::endl;

    // std::cerr << "++++++++++++++++++++++++++++++++++++" << std::endl;
  }

  table_id_t GetTableID() const { return table_id; }

  offset_t GetBaseOff() const { return base_off; }

  uint64_t GetHashBucketSize() const { return SLOT_NUM[table_id] * CVTSize; }

  uint64_t GetBucketNum() const { return bucket_num; }

  HashCore GetHashCore() const { return hash_core; }

  char *GetTablePtr() const { return table_ptr; }

  size_t GetTotalSize() const { return total_size; }

  size_t GetHTInitFVSize() const {
    return bucket_num * (SLOT_NUM[table_id] * CVTSize) +
           init_insert_num * vpkg_size;
  }
  ...
  size_t GetInitFVSize() const { return init_insert_num * vpkg_size; }
  ...
  uint64_t GetInitInsertNum() const { return init_insert_num; }
  ...
  void LocalReadTuple(itemkey_t key);
...
ALWAYS_INLINE
void HashStore::LocalInsertTuple(itemkey_t key, char *value,
                                 size_t value_size) {
  ...
  RDMA_LOG(FATAL) << "Table " << table_id
                  << " alloc a new bucket for key: " << key
                  << ". Current slotnum per bucket: " << SLOT_NUM[table_id];
}

ALWAYS_INLINE
void HashStore::LocalReadTuple(itemkey_t key) {
  uint64_t bkt_pos = GetHash(key, bucket_num, hash_core);
  ...
  for (int i = 0; i < SLOT_NUM[table_id]; i++) {
    CVT *cvt = (CVT *)(cvt_start + i * CVTSize);
    if (cvt->header.key == key) {
      for (int i = 0; i < MAX_VCELL_NUM; ++i) {
        VCell *vcell = &(cvt->vcell[i]);
        RDMA_LOG(INFO) << "vcell[" << i << "]: "
                       << "valid: " << int(vcell->valid)
                       << ", version: " << vcell->version
                       << ", sa: " << int(vcell->sa) << "\n";
      }

      char *full_value =
          cvt->header.remote_full_value_offset + region_start_ptr;
      anchor_t *sa_ptr = (anchor_t *)full_value;
      anchor_t *ea_ptr = (anchor_t *)(full_value + sizeof(anchor_t) +
                                      TABLE_VALUE_SIZE[table_id]);

      RDMA_LOG(INFO) << "Full value: "
                     << "[sa]: " << *sa_ptr
                     << ", [ea]: " << *ea_ptr << "\n";
      return;
    }
