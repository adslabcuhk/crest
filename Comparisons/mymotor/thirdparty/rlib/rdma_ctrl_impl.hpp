...
public:
  RdmaCtrlImpl(int node_id, int tcp_base_port, connection_callback_t callback,
               std::string local_ip)
      : node_id_(node_id), tcp_base_port_(tcp_base_port), local_ip_(local_ip),
  ...
  RNicHandler *open_device(DevIdx idx) {
    ...
    // success open
    {
      rnic = new RNicHandler(idx.dev_id, idx.port_id, ib_ctx, pd, port_attr.lid,
                             DEFAULT_GID_INDEX);
      RDMA_LOG(INFO) << "Initialize RNicHandler with dev_id = " << idx.dev_id
                     << ", name = " << ibv_get_device_name(dev_list[idx.dev_id])
                     << ", "
                     << "port = " << idx.port_id << ", "
                     << "gid = " << DEFAULT_GID_INDEX;
    }
   ...
   */
  template <class T, uint64_t (*F)(QPIdx)> T *get_qp(QPIdx idx) {
    uint64_t key = F(idx);
      ...
      { // in a global critical section
        ...
        case ConnArg::QP: {
          ...
          case IBV_QPT_RC: {
            ...
            // RDMA_LOG(INFO) << "Receive QP request, my node id: " <<
            // arg.payload.qp.from_node << ", client worker id: " <<
            // arg.payload.qp.from_worker;
...
RdmaCtrl::close_device(RNicHandler *rnic) {
...
RdmaCtrl::register_memory(int id, const char *buf, uint64_t size,
                          RNicHandler *rnic, int flag) {
...
RdmaCtrl::get_default_mr(MemoryAttr &attr) {
  return impl_->get_default_mr(attr);
}

inline __attribute__((always_inline)) RCQP *
...
inline __attribute__((always_inline)) UDQP *
...
inline __attribute__((always_inline)) bool
RdmaCtrl::link_symmetric_rcqps(const std::vector<std::string> &cluster,
...
inline __attribute__((always_inline)) void
RdmaCtrl::register_qp_callback(connection_callback_t callback) {
