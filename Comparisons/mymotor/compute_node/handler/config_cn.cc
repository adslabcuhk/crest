...
void Handler::ConfigureComputeNode(int argc, char* argv[]) {
    ...
    std::string s1 =
        "sed -i '5c \"thread_num_per_machine\": " + std::string(argv[2]) + ",' " + config_file;
    ...
    std::string s =
        "sed -i '9c \"iso_level\": " + std::to_string(iso_level_value) + ",' " + config_file;
    system(s.c_str());
}

void Handler::ConfigureComputeNodeForMICRO(int argc, char* argv[]) {
    // ./run_micro <thread_num> <coroutine_num> <access_pattern> <skewness> <write_ratio>
    // <isolation_level>
