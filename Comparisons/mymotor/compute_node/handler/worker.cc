...
extern std::map<int, std::vector<double>> txn_lat_vec;
extern std::vector<int> txn_exec_vec;
extern std::vector<int> txn_validate_vec;
extern std::vector<int> txn_commit_vec;
...
__thread FastRandom *random_generator =
    nullptr; // Per coroutine random generator
...
__thread int *exec_timer;
__thread int *validate_timer;
__thread int *commit_timer;
__thread int *txn_type;
...
void RecordTpLat(double msr_sec) {
  ...
  for (int i = 0; i < total_try_times.size(); i++) {
    tried_count += thread_local_try_times[i];
  }

  double attemp_tput = (double)stat_attempted_tx_total / msr_sec;
  double tx_tput = (double)stat_committed_tx_total / msr_sec;
  double exec_thpt = (double)tried_count / msr_sec;
  ...
  attemp_tp_vec.push_back(exec_thpt);
  tp_vec.push_back(tx_tput);
  medianlat_vec.push_back(percentile_50);
  taillat_vec.push_back(percentile_99);

  // Store the latency to its corresponding vector
  for (int i = 0; i < stat_committed_tx_total; ++i) {
    // Add each element in `timer' into the correponding vector of `txn_lat_vec'
    int committed_txn_type = txn_type[i];
    auto it = txn_lat_vec.find(committed_txn_type);
    if (it == txn_lat_vec.end()) {
      it =
          txn_lat_vec.insert({committed_txn_type, std::vector<double>()}).first;
    }
    it->second.push_back(timer[i]);
  }

  txn_exec_vec.insert(txn_exec_vec.end(), exec_timer,
                      exec_timer + stat_committed_tx_total);
  txn_validate_vec.insert(txn_validate_vec.end(), validate_timer,
                          validate_timer + stat_committed_tx_total);
  txn_commit_vec.insert(txn_commit_vec.end(), commit_timer,
                        commit_timer + stat_committed_tx_total);
...
void RunTATP(coro_yield_t &yield, coro_id_t coro_id, TATP *tatp_c) {
  // Each coroutine has a txn: Each coroutine is a coordinator
  TXN *txn = new TXN(meta_man, qp_man, thread_gid, coro_id, coro_sched,
                     rdma_buffer_allocator, delta_offset_allocator,
                     locked_key_table, addr_cache);
  struct timespec tx_start_time, tx_end_time;
  bool tx_committed = false;

  int tx_exec_id = coro_id - 1;
  int exec_count = 0;
  const int TATP_MAX_EXEC_COUNT = 10;

  // Running transactions
  clock_gettime(CLOCK_REALTIME, &msr_start);
  while (tx_exec_id < tatp_client->txn_run_types->size()) {
    ...
    auto tx_type = tatp_c->txn_run_types->at(tx_exec_id);
    void *param = tatp_c->txn_run_params->at(tx_exec_id);
    // Move to the next transaction parameters
    tx_exec_id += (tatp_c->coro_num - 1);
    ...
    case TATPTxType::kGetSubsciberData: {
      GetSubscriberTxnParam *params = (GetSubscriberTxnParam *)param;
      while (!tx_committed && exec_count++ < TATP_MAX_EXEC_COUNT) {
        tx_committed =
            TxGetSubsciberData(tatp_client, &seed, yield, iter, txn, params);
    ...
    case TATPTxType::kGetNewDestination: {
      // thread_local_try_times[uint64_t(tx_type)]++;
      // tx_committed = TxGetNewDestination(tatp_client, &seed, yield, iter,
      // txn); if (tx_committed)
      //   thread_local_commit_times[uint64_t(tx_type)]++;
      // break;
      GetNewDestinationTxnParam *params = (GetNewDestinationTxnParam *)param;
      while (!tx_committed && exec_count++ < TATP_MAX_EXEC_COUNT) {
        tx_committed =
            TxGetNewDestination(tatp_client, &seed, yield, iter, txn, params);
        thread_local_try_times[uint64_t(tx_type)]++;
        if (tx_committed) {
          thread_local_commit_times[uint64_t(tx_type)]++;
        }
    ...
    case TATPTxType::kGetAccessData: {
      ...
      //   thread_local_commit_times[uint64_t(tx_type)]++;
      // break;
      GetAccessDataTxnParam *params = (GetAccessDataTxnParam *)param;
      while (!tx_committed && exec_count++ < TATP_MAX_EXEC_COUNT) {
        tx_committed =
            TxGetAccessData(tatp_client, &seed, yield, iter, txn, params);
        thread_local_try_times[uint64_t(tx_type)]++;
        if (tx_committed) {
          thread_local_commit_times[uint64_t(tx_type)]++;
        }
    ...
    case TATPTxType::kUpdateSubscriberData: {
      // thread_local_try_times[uint64_t(tx_type)]++;
      // tx_committed =
      //     TxUpdateSubscriberData(tatp_client, &seed, yield, iter, txn);
      // if (tx_committed)
      //   thread_local_commit_times[uint64_t(tx_type)]++;
      // break;
      UpdateSubscriberDataTxnParam *params =
          (UpdateSubscriberDataTxnParam *)param;
      while (!tx_committed && exec_count++ < TATP_MAX_EXEC_COUNT) {
        tx_committed = TxUpdateSubscriberData(tatp_client, &seed, yield, iter,
                                              txn, params);
        thread_local_try_times[uint64_t(tx_type)]++;
        if (tx_committed) {
          thread_local_commit_times[uint64_t(tx_type)]++;
    ...
    case TATPTxType::kUpdateLocation: {
      ...
      //   thread_local_commit_times[uint64_t(tx_type)]++;
      // break;
      UpdateLocationTxnParam *params = (UpdateLocationTxnParam *)param;
      while (!tx_committed && exec_count++ < TATP_MAX_EXEC_COUNT) {
        tx_committed =
            TxUpdateLocation(tatp_client, &seed, yield, iter, txn, params);
        thread_local_try_times[uint64_t(tx_type)]++;
        if (tx_committed) {
          thread_local_commit_times[uint64_t(tx_type)]++;
        }
    ...
    case TATPTxType::kInsertCallForwarding: {
      // thread_local_try_times[uint64_t(tx_type)]++;
      // tx_committed =
      //     TxInsertCallForwarding(tatp_client, &seed, yield, iter, txn);
      // if (tx_committed)
      //   thread_local_commit_times[uint64_t(tx_type)]++;
      // break;
      InsertCallForwardingTxnParam *params =
          (InsertCallForwardingTxnParam *)param;
      while (!tx_committed && exec_count++ < TATP_MAX_EXEC_COUNT) {
        tx_committed = TxInsertCallForwarding(tatp_client, &seed, yield, iter,
                                              txn, params);
        thread_local_try_times[uint64_t(tx_type)]++;
        if (tx_committed) {
          thread_local_commit_times[uint64_t(tx_type)]++;
    ...
    case TATPTxType::kDeleteCallForwarding: {
      thread_local_try_times[uint64_t(tx_type)]++;
      tx_committed =
          TxDeleteCallForwarding(tatp_client, &seed, yield, iter, txn);
      if (tx_committed)
        thread_local_commit_times[uint64_t(tx_type)]++;
      break;
    }
    ...
    if (tx_committed) {
      ...
      txn_type[stat_committed_tx_total++] = (int)tx_type;
    }
...
void RunSmallBank(coro_yield_t &yield, coro_id_t coro_id,
  ...
  int tx_exec_id = coro_id - 1;
  int exec_count = 0;
  const int SMALLBANK_MAX_EXEC_COUNT = 10;

  int txn_exec_time = 0;
  int txn_validate_time = 0;
  int txn_commit_time = 0;

  // Running transactions
  clock_gettime(CLOCK_REALTIME, &msr_start);
  while (tx_exec_id < smallbank_client->txn_run_types->size()) {
    auto tx_type = smallbank_client->txn_run_types->at(tx_exec_id);
    void *param = smallbank_client->txn_run_params->at(tx_exec_id);
    // Move to the next transaction parameters
    tx_exec_id += (smallbank_client->coro_num - 1);
    // SmallBankTxType tx_type = smallbank_workgen_arr[FastRand(&seed) % 100];
    uint64_t iter = ++tx_id_generator; // Global atomic transaction id
    stat_attempted_tx_total++;
    ...
    case SmallBankTxType::kAmalgamate: {
      AmalgamateParam *txn_params = (AmalgamateParam *)param;
      while (!tx_committed && exec_count++ < SMALLBANK_MAX_EXEC_COUNT) {
        tx_committed =
            TxAmalgamate(smallbank_client, &seed, yield, iter, txn, txn_params);
        thread_local_try_times[uint64_t(tx_type)]++;
        txn_exec_time += txn->GetTxnExecTime();
        txn_validate_time += txn->GetValidateAndLogTime();
        txn_commit_time += txn->GetCommitTime();
        if (tx_committed)
          thread_local_commit_times[uint64_t(tx_type)]++;
      }
      break;
    ...
    case SmallBankTxType::kBalance: {
      BalanceParam *txn_params = (BalanceParam *)param;
      while (!tx_committed && exec_count++ < SMALLBANK_MAX_EXEC_COUNT) {
        tx_committed =
            TxBalance(smallbank_client, &seed, yield, iter, txn, txn_params);
        thread_local_try_times[uint64_t(tx_type)]++;
        txn_exec_time += txn->GetTxnExecTime();
        txn_validate_time += txn->GetValidateAndLogTime();
        txn_commit_time += txn->GetCommitTime();
    ...
    case SmallBankTxType::kDepositChecking: {
      DepositCheckingParam *txn_params = (DepositCheckingParam *)param;
      while (!tx_committed && exec_count++ < SMALLBANK_MAX_EXEC_COUNT) {
        tx_committed = TxDepositChecking(smallbank_client, &seed, yield, iter,
                                         txn, txn_params);
        thread_local_try_times[uint64_t(tx_type)]++;
        txn_exec_time += txn->GetTxnExecTime();
        txn_validate_time += txn->GetValidateAndLogTime();
    ...
    case SmallBankTxType::kSendPayment: {
      SendPaymentParam *txn_params = (SendPaymentParam *)param;
      while (!tx_committed && exec_count++ < SMALLBANK_MAX_EXEC_COUNT) {
    ...
    case SmallBankTxType::kTransactSaving: {
      TransactSavingParam *txn_params = (TransactSavingParam *)param;
      while (!tx_committed && exec_count++ < SMALLBANK_MAX_EXEC_COUNT) {
        tx_committed = TxTransactSaving(smallbank_client, &seed, yield, iter,
                                        txn, txn_params);
        thread_local_try_times[uint64_t(tx_type)]++;
        txn_exec_time += txn->GetTxnExecTime();
        txn_validate_time += txn->GetValidateAndLogTime();
    ...
    case SmallBankTxType::kWriteCheck: {
      WriteCheckParam *txn_params = (WriteCheckParam *)param;
      while (!tx_committed && exec_count++ < SMALLBANK_MAX_EXEC_COUNT) {
        tx_committed =
            TxWriteCheck(smallbank_client, &seed, yield, iter, txn, txn_params);
        thread_local_try_times[uint64_t(tx_type)]++;
        txn_exec_time += txn->GetTxnExecTime();
        txn_validate_time += txn->GetValidateAndLogTime();
        txn_commit_time += txn->GetCommitTime();
    ...
    if (tx_committed) {
      ...
      double tx_usec =
          (tx_end_time.tv_sec - tx_start_time.tv_sec) * 1000000 +
          (double)(tx_end_time.tv_nsec - tx_start_time.tv_nsec) / 1000;
      // (kqh): We store the latency and transaction type
      timer[stat_committed_tx_total] = tx_usec;
      exec_timer[stat_committed_tx_total] = txn_exec_time;
      validate_timer[stat_committed_tx_total] = txn_validate_time;
      commit_timer[stat_committed_tx_total] = txn_commit_time;
      txn_type[stat_committed_tx_total++] = (int)tx_type;
    }
  }
  // if (stat_attempted_tx_total == smallbank_client->txn_run_types->size()) {
  // A coroutine calculate the total execution time and exits
  clock_gettime(CLOCK_REALTIME, &msr_end);
  // double msr_usec = (msr_end.tv_sec - msr_start.tv_sec) * 1000000 +
  // (double) (msr_end.tv_nsec - msr_start.tv_nsec) / 1000;
...
void RunTPCC(coro_yield_t &yield, coro_id_t coro_id, int finished_num) {
  ...
  while (true) {
    ...
    stat_attempted_tx_total++;
    // TLOG(INFO, thread_gid) << "Thread " << thread_gid << " attemps txn " <<
    // stat_attempted_tx_total << " txn id: " << iter;
    ...
    case TPCCTxType::kDelivery: {
      ...
      thread_local_try_times[uint64_t(tx_type)]++;
      tx_committed =
          TxDelivery(tpcc_client, random_generator, yield, iter, txn);
      if (tx_committed)
        thread_local_commit_times[uint64_t(tx_type)]++;
      break;
    }
    case TPCCTxType::kNewOrder: {
      NewOrderParam param;
      while (!tx_committed && exec_count < MAX_EXEC_COUNT) {
        thread_local_try_times[uint64_t(tx_type)]++;
        ++exec_count;
        uint64_t iter = ++tx_id_generator; // Global atomic transaction id
        tx_committed =
            TxNewOrder(tpcc_client, random_generator, yield, iter, txn, &param);
        txn_exec_time += (txn->GetTxnExecTime());
        txn_validate_time += (txn->GetValidateAndLogTime());
        txn_commit_time += (txn->GetCommitTime());
        if (tx_committed) {
    ...
    case TPCCTxType::kOrderStatus: {
      ...
      uint64_t iter = ++tx_id_generator; // Global atomic transaction id
      tx_committed =
          TxOrderStatus(tpcc_client, random_generator, yield, iter, txn);
      txn_exec_time += (txn->GetTxnExecTime());
      txn_validate_time += (txn->GetValidateAndLogTime());
      txn_commit_time += (txn->GetCommitTime());
      if (tx_committed)
    ...
    case TPCCTxType::kPayment: {
      ...
      while (!tx_committed && exec_count < MAX_EXEC_COUNT) {
        ...
        uint64_t iter = ++tx_id_generator; // Global atomic transaction id
        tx_committed =
            TxPayment(tpcc_client, random_generator, yield, iter, txn, &param);
        txn_exec_time += (txn->GetTxnExecTime());
        txn_validate_time += (txn->GetValidateAndLogTime());
        txn_commit_time += (txn->GetCommitTime());
        if (tx_committed) {
    ...
    case TPCCTxType::kStockLevel: {
      ...
      //   tx_committed = TxStockLevel(tpcc_client, random_generator, yield,
      //   iter, txn); if (!tx_committed) {
      ...
      uint64_t iter = ++tx_id_generator; // Global atomic transaction id
      tx_committed =
          TxStockLevel(tpcc_client, random_generator, yield, iter, txn);
      if (tx_committed)
        thread_local_commit_times[uint64_t(tx_type)]++;
      break;
    }
    ...
    if (tx_committed) {
      ...
      //     (tx_end_time.tv_sec - tx_start_time.tv_sec) * 1000000 +
      //     (double)(tx_end_time.tv_nsec - tx_start_time.tv_nsec) / 1000;
      double tx_usec = dura.count();
      // (kqh): We store the latency of this transaction execution, and the type
      // of transaction it executes
      timer[stat_committed_tx_total] = tx_usec;
      exec_timer[stat_committed_tx_total] = txn_exec_time;
      validate_timer[stat_committed_tx_total] = txn_validate_time;
      commit_timer[stat_committed_tx_total] = txn_commit_time;
    ...
    if (to_crash[thread_local_id]) {
      ...
      //     RDMA_LOG(INFO) << "cid: " << i << ", txid: " <<
      //     locked_key_table[i].tx_id << ", num_entry: " <<
      //     locked_key_table[i].num_entry;
      //   }
      // }
    ...
    if (probe[thread_local_id]) {
      ...
      msr_sec = (msr_end.tv_sec - last_end.tv_sec) +
                (double)(msr_end.tv_nsec - last_end.tv_nsec) / 1000000000;
      last_end = msr_end;

      attemp_tput =
          (double)(stat_attempted_tx_total - last_stat_attempted_tx_total) /
          msr_sec;
      last_stat_attempted_tx_total = stat_attempted_tx_total;

      tx_tput =
          (double)(stat_committed_tx_total - last_stat_committed_tx_total) /
          msr_sec;
      last_stat_committed_tx_total = stat_committed_tx_total;

      tp_probe_list->emplace_back(
          TpProbe{.ctr = probe_times, .tp = tx_tput, .attemp_tp = attemp_tput});
...
void RunMICRO(coro_yield_t &yield, coro_id_t coro_id, MICRO *micro) {
  ...
  int tx_exec_id = coro_id - 1;
  int exec_count = 0;
  const int MICRO_MAX_EXEC_COUNT = 10;
  ...
  clock_gettime(CLOCK_REALTIME, &msr_start);
  itemkey_t key;
  while (tx_exec_id < micro->txn_run_types->size()) {
    auto tx_type = micro->txn_run_types->at(tx_exec_id);
    void *param = micro->txn_run_params->at(tx_exec_id);
    tx_exec_id += (micro->coro_num - 1);
    ...
    case MicroTxType::kUpdateOne: {
      RWUpdateOneParam *txn_params = (RWUpdateOneParam *)param;
      while (!tx_committed && exec_count++ < MICRO_MAX_EXEC_COUNT) {
        tx_committed = TxUpdateOne(yield, iter, txn, key, txn_params);
        thread_local_try_times[uint64_t(tx_type)]++;
        txn_exec_time += (txn->GetTxnExecTime());
        txn_validate_time += (txn->GetValidateAndLogTime());
        txn_commit_time += (txn->GetCommitTime());
    ...
    case MicroTxType::kReadOne: {
      RWReadOneParam *txn_params = (RWReadOneParam *)param;
      while (!tx_committed && exec_count++ < MICRO_MAX_EXEC_COUNT) {
        tx_committed = TxReadOne(yield, iter, txn, key, txn_params);
        thread_local_try_times[uint64_t(tx_type)]++;
        if (tx_committed)
          thread_local_commit_times[uint64_t(tx_type)]++;
      }
...
void run_thread(thread_params *params, TATP *tatp_cli, SmallBank *smallbank_cli,
  ...
  txn_type = new int[ATTEMPTED_NUM]();
  exec_timer = new int[ATTEMPTED_NUM]();
  validate_timer = new int[ATTEMPTED_NUM]();
  commit_timer = new int[ATTEMPTED_NUM]();
  ...
  for (coro_id_t coro_i = 0; coro_i < coro_num; coro_i++) {
    ...
    uint64_t coro_seed = thread_gid;
    // Use Time-based random seed
    auto t = time(nullptr);
    char d[16];
    *(uint64_t *)d = t;
    *(uint64_t *)(d + 8) = coro_seed;
    // Use current time as the seed of random generator
    uint64_t seed = SuperFastHash(d, 16);
    random_generator[coro_i].SetSeed(seed);
    ...
    if (coro_i == POLL_ROUTINE_ID) {
      ...
      if (bench_name == "tatp") {
        coro_sched->coro_array[coro_i].func =
            coro_call_t(bind(RunTATP, _1, coro_i, tatp_cli));
      } else if (bench_name == "smallbank") {
        smallbank_cli->coro_num = coro_num;
        coro_sched->coro_array[coro_i].func =
            coro_call_t(bind(RunSmallBank, _1, coro_i, smallbank_cli));
      } else if (bench_name == "tpcc") {
        coro_sched->coro_array[coro_i].func =
            coro_call_t(bind(RunTPCC, _1, coro_i, 0));
      } else if (bench_name == "micro") {
        micro_cli->coro_num = coro_num;
        coro_sched->coro_array[coro_i].func =
            coro_call_t(bind(RunMICRO, _1, coro_i, micro_cli));
      }
    }
  }
...
void recovery(thread_params *params, TATP *tatp_cli, SmallBank *smallbank_cli,
              TPCC *tpcc_cli, MICRO *micro_cli, int finished_num,
              std::vector<TpProbe> *thread_tp_probe, t_id_t crasher) {
  auto bench_name = params->bench_name;
  std::string config_filepath =
      "../../../config/" + bench_name + "_config.json";
  ...
  // RDMA_LOG(INFO) << "Thread " << params->thread_global_id << " starts
  // recover. Builinding connection...";
  ...
  for (int i = 0; i < coro_num; i++) {
    ...
    if (num_entry) {
      // TLOG(INFO, thread_gid) << "coro: " << i << " op_log_size (B): " <<
      // sizeof(tx_id_t) + sizeof(num_entry) + num_entry *
      // sizeof(LockedKeyEntry);
      for (int j = 0; j < num_entry; j++) {
        char *cas_buf = rdma_buffer_allocator->Alloc(sizeof(lock_t));
        *(lock_t *)cas_buf = 0;

        auto *qp =
            qp_man->GetRemoteDataQPWithNodeID(target[i].entries[j].remote_node);
        qp->post_cas(cas_buf, target[i].entries[j].remote_off, target[i].tx_id,
                     0, 0);
      }
    }
  }
