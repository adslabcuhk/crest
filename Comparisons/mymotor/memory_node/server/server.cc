...
void Server::AllocMem() {
  ...
  if (use_pm) {
    ...
    // sys/mman.h: MAP_SHARED_VALIDATE, MAP_SYNC
    mem_region = (char *)mmap(0, data_size + delta_size, PROT_READ | PROT_WRITE,
                              0x80003, pm_file_fd, 0);
...
void Server::InitRDMA() {
  ...
  int open_dev_id = 0;
  if (server_node_id == 0) {
    // For node: 10.118.0.45 (MN-0), the device id should be 1, i.e., mlx5_0
    open_dev_id = 0;
  } else if (server_node_id == 1) {
    // For node: 10.118.0.49 (MN-1), the device id should be 0, i.e., mlx5_1
    open_dev_id = 0;
  }
  RdmaCtrl::DevIdx idx{.dev_id = open_dev_id,
                       .port_id = 1}; // using the first RNIC's first port
  rdma_ctrl->open_thread_local_device(idx);
  RDMA_ASSERT(rdma_ctrl->register_memory(SERVER_HASH_BUFF_ID, mem_region,
...
void Server::ConnectMN() {
  ...
  for (size_t index = 0; index < other_mn_ips.size(); index++) {
    ...
    while (QP::get_remote_mr(remote_ip, remote_port, SERVER_HASH_BUFF_ID,
                             &remote_mr) != SUCC) {
      usleep(2000);
    }

    RDMA_LOG(INFO) << "Get MR of MN ID: " << remote_node_id
    ...
    // This number should be larger than the total amount of CN threads in the
    // cluster
    RCQP *data_qp =
        rdma_ctrl->create_rc_qp(create_rc_idx(remote_node_id, 1000 + my_mn_id),
                                rdma_ctrl->get_device(), &local_mr);

    ConnStatus rc;
    do {
      rc = data_qp->connect(remote_ip, remote_port);
      if (rc == SUCC) {
        // Bind the hash mr as the default remote mr for convenient parameter
        // passing
...
void Server::LoadData(node_id_t machine_id,
  ...
  // Init tables
  MemStoreAllocParam mem_store_alloc_param(mem_region, hash_buffer, 0,
                                           mem_region + data_size);
  ...
  if (workload == "TATP") {
    ...
    tatp_server->LoadTable(machine_id, machine_num, &mem_store_alloc_param,
                           total_size, ht_loadfv_size, ht_size, initfv_size,
                           real_cvt_size);
    ...
    smallbank_server->LoadTable(machine_id, machine_num, &mem_store_alloc_param,
                                total_size, ht_loadfv_size, ht_size,
                                initfv_size, real_cvt_size);
  ...
  // std::cerr << (double)total_size / 1024.0 / 1024.0 << " " << (double)ht_size
  // / 1024.0 / 1024.0 << " " << (double)real_cvt_size / 1024.0 / 1024.0 << " "
  // << (double)initfv_size / 1024.0 / 1024.0 << std::endl; std::cerr <<
  // "----------------------------------------------------------" << std::endl;

  std::cerr << "Data area: " << (double)data_size / 1024.0 / 1024.0 << " MB"
            << std::endl;
  std::cerr << "Delta area: " << (double)delta_size / 1024.0 / 1024.0 << " MB"
            << std::endl;
  std::cerr << "----------------------------------------------------------"
            << std::endl;
  ...
  // std::string mem_ft_res = "../../../mem_ft/" + workload +
  // "_ht_loadedfv_size.txt";

  // std::ofstream of;
  // of.open(mem_ft_res.c_str(), std::ios::app);
  // of << MAX_VCELL_NUM << " " << (double)ht_loadfv_size / 1024.0 / 1024.0 <<
  // std::endl; of.close();
  ...
  // of_ft << std::endl;
  // of_ft << "----------------------------------------------------------" <<
  // std::endl; of_ft << workload << " VNum: " << MAX_VCELL_NUM << std::endl;
  // of_ft << "----------------------------------------------------------" <<
  // std::endl; of_ft << "Total HT LoadCVT LoadFV (MB)" << std::endl; of_ft <<
  // (double)total_size / 1024.0 / 1024.0 << " " << (double)ht_size / 1024.0 /
  // 1024.0 << " " << (double)real_cvt_size / 1024.0 / 1024.0 << " " <<
  // (double)initfv_size / 1024.0 / 1024.0 << std::endl; of_ft.close();
...
void Server::SendMeta(node_id_t machine_id, std::string &workload,
                      size_t compute_node_num, offset_t delta_start_off,
  ...
  PrepareHashMeta(machine_id, workload, &hash_meta_buffer, total_meta_size,
                  delta_start_off, per_thread_delta_size);
...
void Server::PrepareHashMeta(node_id_t machine_id, std::string &workload,
  ...
  for (auto &hash_table : all_priamry_tables) {
    auto *hash_meta = new HashMeta(
        hash_table->GetTableID(), (uint64_t)hash_table->GetTablePtr(),
        hash_table->GetBaseOff(), hash_table->GetBucketNum(),
        hash_table->GetHashBucketSize(), hash_table->GetHashCore());
  ...
  total_meta_size =
      ...
      sizeof(machine_id) + sizeof(delta_start_off) +
      sizeof(per_thread_delta_size) + primary_hash_meta_num * hash_meta_len +
...
void Server::AcceptReq() {
  ...
  RDMA_LOG(INFO) << "[AcceptReq] IsPrimaryFail: " << is_primary_fail
                 << ". I migrate table " << table_id << " from me to MN "
                 << target_mn_id;
  ...
  for (int i = 0; i < tables.size(); i++) {
    if (tables[i]->GetTableID() == table_id) {
      ...
      other_mn_qps[target_mn_id]->post_send(
          IBV_WR_RDMA_WRITE, start_copy, migration_size,
          tables[i]->GetBaseOff(), IBV_SEND_SIGNALED);
        ...
        for (int j = 0; j < SLOT_NUM[table_id]; j++) {
          ...
          if (cvt->header.value_size) {
            if (cvt->header.remote_attribute_offset != UN_INIT_POS) {
              ...
              char *local_attr_addr =
                  mem_region + cvt->header.remote_attribute_offset;

              other_mn_qps[target_mn_id]->post_send(
                  IBV_WR_RDMA_WRITE, local_attr_addr, ATTR_BAR_SIZE[table_id],
                  cvt->header.remote_attribute_offset, IBV_SEND_SIGNALED);
            ...
            if (cvt->header.user_inserted) {
              size_t vpkg_size =
                  TABLE_VALUE_SIZE[table_id] + sizeof(anchor_t) * 2;
              migration_size += vpkg_size;
              char *local_fv_addr =
                  mem_region + cvt->header.remote_full_value_offset;

              other_mn_qps[target_mn_id]->post_send(
                  IBV_WR_RDMA_WRITE, local_fv_addr, vpkg_size,
                  cvt->header.remote_full_value_offset, IBV_SEND_SIGNALED);
              write_cnt++;
  ...
  RDMA_LOG(INFO) << "[AcceptReq] Migrate SUCCESS: "
                 << (double)migration_size / 1024.0 << " KB."
                 << " Write cnt: " << write_cnt
                 << ". new_attr_bar_cnt: " << new_attr_bar_cnt
                 << ". new_insert_cnt: " << new_insert_cnt;
...
void Server::OutputMemoryFootprint(std::string &workload) {
  ...
  std::cerr << "TotalCVT (MB): " << (double)total_cvt_size / 1024.0 / 1024.0
            << std::endl;
  std::cerr << "----------------------------------------------------------"
            << std::endl;
  ...
  of_ft << "TotalCVT (MB): " << (double)total_cvt_size / 1024.0 / 1024.0
        << std::endl;
  of_ft.close();
}
...
bool Server::Run(std::string &workload) {
  ...
  while (true) {
    ...
    if (ch == 'q') {
      ...
      table_id_t table_id;
      itemkey_t item_key;
      scanf("%lu %lu", &table_id, &item_key);
#if WORKLOAD_TPCC
      tpcc_server->GetPrimaryHashStore()[(size_t)table_id]->LocalReadTuple(
          item_key);
#else
...
int main(int argc, char *argv[]) {
  ...
  auto max_client_num_per_mn =
      local_node.get("max_client_num_per_mn").get_uint64();
  auto per_thread_delta_size_MB =
      local_node.get("per_thread_delta_size_MB").get_uint64();
  ...
  auto server =
      std::make_shared<Server>(machine_id, local_port, local_meta_port,
                               data_size, delta_size, use_pm, pm_file);
  ...
  RDMA_LOG(INFO) << "[Initialize Memory Node Time]: " << dura.count() << " ms";
  server->SendMeta(machine_id, workload, compute_node_num, data_size,
                   per_thread_delta_size);
