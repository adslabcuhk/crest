...
void Server::AllocMem() {
    ...
    if (use_pm) {
        ...
        mem_region =
            (char *)mmap(0, data_size + delta_size, PROT_READ | PROT_WRITE, 0x80003, pm_file_fd, 0);
...
void Server::InitRDMA() {
    ...
    RdmaCtrl::DevIdx idx;
    idx.dev_id = local_node.get("ib_dev_id").get_int64();
    idx.port_id = local_node.get("ib_port_id").get_int64();
    rdma_ctrl->open_thread_local_device(idx);
    RDMA_ASSERT(rdma_ctrl->register_memory(SERVER_HASH_BUFF_ID, mem_region, data_size + delta_size,
...
void Server::ConnectMN() {
    ...
    for (size_t index = 0; index < other_mn_ips.size(); index++) {
        ...
        RDMA_LOG(INFO) << "Get MR of MN ID: " << remote_node_id << " IP: " << remote_ip
                       << " PORT: " << remote_port << " Success!";
        ...
        do {
            ...
            if (rc == SUCC) {
                ...
                RDMA_LOG(INFO) << "Connect QP with MN ID: " << remote_node_id
                               << " IP: " << remote_ip << " PORT: " << remote_port << " Success!";
            }
            usleep(2000);
        } while (rc != SUCC);
    }
...
void Server::LoadData(node_id_t machine_id,
    ...
    if (workload == "TATP") {
        ...
        tatp_server->LoadTable(machine_id, machine_num, &mem_store_alloc_param, total_size,
                               ht_loadfv_size, ht_size, initfv_size, real_cvt_size);
    } else if (workload == "SmallBank") {
        smallbank_server = new SmallBank();
        smallbank_server->LoadTable(machine_id, machine_num, &mem_store_alloc_param, total_size,
                                    ht_loadfv_size, ht_size, initfv_size, real_cvt_size);
    } else if (workload == "TPCC") {
        tpcc_server = new TPCC();
        tpcc_server->LoadTable(machine_id, machine_num, &mem_store_alloc_param, total_size,
                               ht_loadfv_size, ht_size, initfv_size, real_cvt_size);
    } else if (workload == "MICRO") {
        micro_server = new MICRO();
        micro_server->LoadTable(machine_id, machine_num, &mem_store_alloc_param, total_size,
                                ht_loadfv_size, ht_size, initfv_size, real_cvt_size);
    }
...
void Server::SendMeta(node_id_t machine_id, std::string &workload, size_t compute_node_num,
    ...
    PrepareHashMeta(machine_id, workload, &hash_meta_buffer, total_meta_size, delta_start_off,
...
void Server::PrepareHashMeta(node_id_t machine_id, std::string &workload, char **hash_meta_buffer,
    ...
    for (auto &hash_table : all_priamry_tables) {
        auto *hash_meta = new HashMeta(
            hash_table->GetTableID(), (uint64_t)hash_table->GetTablePtr(), hash_table->GetBaseOff(),
            hash_table->GetBucketNum(), hash_table->GetHashBucketSize(), hash_table->GetHashCore());
    ...
    total_meta_size = sizeof(primary_hash_meta_num) + sizeof(backup_hash_meta_num) +
                      sizeof(machine_id) + sizeof(delta_start_off) + sizeof(per_thread_delta_size) +
                      primary_hash_meta_num * hash_meta_len + backup_hash_meta_num * hash_meta_len +
...
void Server::AcceptReq() {
    ...
    RDMA_LOG(INFO) << "[AcceptReq] IsPrimaryFail: " << is_primary_fail << ". I migrate table "
                   << table_id << " from me to MN " << target_mn_id;
    ...
    for (int i = 0; i < tables.size(); i++) {
        if (tables[i]->GetTableID() == table_id) {
            ...
            other_mn_qps[target_mn_id]->post_send(IBV_WR_RDMA_WRITE, start_copy, migration_size,
    ...
    RDMA_LOG(INFO) << "[AcceptReq] Migrate SUCCESS: " << (double)migration_size / 1024.0 << " KB."
                   << " Write cnt: " << write_cnt << ". new_attr_bar_cnt: " << new_attr_bar_cnt
...
#if WORKLOAD_TPCC
            tpcc_server->GetPrimaryHashStore()[(size_t)table_id]->LocalReadTuple(item_key);
#else
...
int main(int argc, char *argv[]) {
    ...
    auto dura = std::chrono::duration_cast<std::chrono::milliseconds>(end_ts - start_ts);
