...
void Handler::ConfigureComputeNode(int argc, char *argv[]) {
    ...
    if (argc == 5) {
        std::string s1 =
            "sed -i '5c \"thread_num_per_machine\": " + std::string(argv[3]) + ",' " + config_file;
        std::string s2 =
            "sed -i '6c \"coroutine_num\": " + std::string(argv[4]) + ",' " + config_file;
        system(s1.c_str());
        system(s2.c_str());
    }
    ...
    std::string s =
        "sed -i '8c \"txn_system\": " + std::to_string(txn_system_value) + ",' " + config_file;
    system(s.c_str());
    return;
}
...
void Handler::GenThreads(std::string bench_name) {
    ...
    RDMA_LOG(INFO) << "Alloc local memory: "
                   << (size_t)(thread_num_per_machine * PER_THREAD_ALLOC_SIZE) / (1024 * 1024)
                   << " MB. Waiting...";
    ...
    if (bench_name == "smallbank") {
        ...
        FastRandom r;
        ZipfGenerator zipf(smallbank_client->num_accounts_global, smallbank_client->zipfian);
        ...
        for (int i = 0; i < kTxnNum; ++i) {
            ...
            switch (t) {
                ...
                case TATPTxType::kUpdateSubscriberData: {
                    UpdateSubscriberDataTxnParam *param = new UpdateSubscriberDataTxnParam();
                    param->s_id_ = tatp_client->GetNonUniformRandomSubscriber();
                    param->sf_type_ = fast_randoms[tid].RandNumber(0, 3);
                    tatp_txn_params[tid].push_back(param);
                    break;
                ...
                case TATPTxType::kInsertCallForwarding: {
                    InsertCallForwardingTxnParam *param = new InsertCallForwardingTxnParam();
                    param->s_id_ = tatp_client->GetNonUniformRandomSubscriber();
                    param->sf_type_ = fast_randoms[tid].RandNumber(0, 3);
                ...
                case TATPTxType::kDeleteCallForwarding: {
                    DeleteCallForwardingTxnParam *param = new DeleteCallForwardingTxnParam();
                    tatp_txn_params[tid].push_back(param);
                    break;
                }
                default:
    ...
    for (t_id_t i = 0; i < thread_num_per_machine; i++) {
        ...
        smallbank_c->txn_run_types = &smallbank_txn_param_type[param_arr[i].thread_global_id];
        smallbank_c->txn_run_params = &smallbank_txn_params[param_arr[i].thread_global_id];

        MICRO *micro_c = new MICRO();
        micro_c->coro_num = coro_num;
        micro_c->txn_run_types = &micro_txn_param_type[param_arr[i].thread_global_id];
        ...
        thread_arr[i] =
            std::thread(run_thread, &param_arr[i], tatp_c, smallbank_c, tpcc_client, micro_c);
...
void Handler::OutputResult(std::string bench_name, std::string system_name) {
    ...
    of_detail << total_attemp_tp << " " << total_tp << " " << medianlat_vec[0] << " "
              << medianlat_vec[thread_num - 1] << " " << avg_median << " " << taillat_vec[0] << " "
              << taillat_vec[thread_num - 1] << " " << avg_tail << std::endl;
    ...
    if (bench_name == "tatp") {
        ...
        of_abort_rate << "All Tried: " << all_tried_count
                      ...
                      << (double)(all_tried_count - all_committed_count) / (double)all_tried_count
                      << std::endl;
    } else if (bench_name == "tpcc") {
        uint64_t all_tried_count = 0, all_committed_count = 0;
    ...
    std::cerr << system_name << " " << total_attemp_tp / 1000 << " " << total_tp / 1000 << " "
              << avg_median << " " << avg_tail << std::endl;
    ...
    if (bench_name == "tatp") {
        ...
        of << "All Tried: " << all_tried_count << ", All committed: " << all_committed_count
           << ", Abort Rate: "
           << (double)(all_tried_count - all_committed_count) / (double)all_tried_count
           << std::endl;
    } else if (bench_name == "tpcc") {
    ...
    for (const auto &entry : tx_lat_vec) {
        ...
        total_latency.insert(total_latency.end(), latencies.begin(), latencies.end());
        ...
        printf(
            "TxnType: %d, AvgLat: %.2f us, P50: %.2f us, P99: %.2f us, P999: "
            "%.2f us\n",
    ...
    uint64_t validate_total = std::accumulate(validate_vec.begin(), validate_vec.end(), 0);
    ...
    uint64_t commit_total = std::accumulate(commit_vec.begin(), commit_vec.end(), 0);
    ...
    for (size_t i = 0; i < avg_lantecy.size(); ++i) {
        of_latency << "TxnType: " << i << ", AvgLat: " << avg_lantecy[i]
                   << " us, P50: " << p50_lantecy[i] << " us, P99: " << p99_lantecy[i]
    ...
    of_latency << "Total Latency: AvgLat: " << total_latency_avg
               << " us, P50: " << total_latency_p50 << " us, P99: " << total_latency_p99
               << " us, P999: " << total_latency_p999 << " us\n";

    of_latency << "Exec: Avg: " << exec_avg << " us, P50: " << exec_p50 << " us, P99: " << exec_p99
               << " us, P999: " << exec_p999 << " us\n";

    of_latency << "Validate: Avg: " << validate_avg << " us, P50: " << validate_p50
               << " us, P99: " << validate_p99 << " us, P999: " << validate_p999 << " us\n";

    of_latency << "Commit: Avg: " << commit_avg << " us, P50: " << commit_p50
               << " us, P99: " << commit_p99 << " us, P999: " << commit_p999 << " us\n";

    of_latency.close();

    std::cout << "Total Latency: AvgLat: " << total_latency_avg << " us, P50: " << total_latency_p50
              << " us, P99: " << total_latency_p99 << " us, P999: " << total_latency_p999
              << " us\n";

    std::cout << "Exec: Avg: " << exec_avg << " us, P50: " << exec_p50 << " us, P99: " << exec_p99
              << " us, P999: " << exec_p999 << " us\n";
    ...
    of << system_name << " " << total_attemp_tp / 1000 << " " << total_tp / 1000 << " "
       << total_latency_avg << " " << total_latency_p50 << " " << total_latency_p99 << " "
       << total_latency_p999
       << " "
       // Dump the latency breakdown results
       << exec_avg << " " << validate_avg << " " << commit_avg << std::endl;
    ...
    if (bench_name == "MICRO") {
        ...
        std::cerr << system_name << " avg_lock_dur: " << total_lock_dur / lock_durations.size()
                  << std::endl;
    }
#endif
}
...
void Handler::GenThreadsForMICRO() {
    ...
    if (coro_num < 10) {
        thread_num_coro_num =
            std::to_string(thread_num_per_machine) + "_0" + std::to_string(coro_num);
    } else {
        thread_num_coro_num =
    ...
    for (t_id_t i = 0; i < thread_num_per_machine; i++) {
        ...
        thread_arr[i] = std::thread(run_thread, &param_arr[i], nullptr, nullptr, nullptr, nullptr);
