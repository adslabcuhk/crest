...
MetaManager::MetaManager() {
    ...
    auto remote_meta_ports =
        pm_nodes.get("remote_meta_ports");  // Array Used for transferring datastore metas
    ...
    for (size_t index = 0; index < remote_ips.size(); index++) {
        ...
        if (remote_machine_id == -1) {
            std::cerr << "Thread " << std::this_thread::get_id()
                      << " GetMemStoreMeta() failed!, remote_machine_id = -1" << std::endl;
        }
        int remote_port = (int)remote_ports.get(index).get_int64();
        remote_nodes.push_back(
            RemoteNode{.node_id = remote_machine_id, .ip = remote_ip, .port = remote_port});
    }
    ...
    // (kqh): Use the second RNIC: mlx5_0, it's for all CNs
    RdmaCtrl::DevIdx idx;
...
node_id_t MetaManager::GetMemStoreMeta(std::string& remote_ip, int remote_port) {
    ...
    if ((*((uint64_t*)eof)) == MEM_STORE_META_END) {
        for (size_t i = 0; i < primary_meta_num; i++) {
            ...
            // RDMA_LOG(INFO) << "primary_node_ip: " << remote_ip << " table id: " << meta.table_id
            // << " data_ptr: 0x" << std::hex << meta.data_ptr << " base_off: 0x" << meta.base_off
            // << " bucket_num: " << std::dec << meta.bucket_num << " node_size: " << meta.node_size
            // << " B";
        }
        ...
        for (size_t i = 0; i < backup_meta_num; i++) {
            ...
            // RDMA_LOG(INFO) << "backup_node_ip: " << remote_ip << " table id: " << meta.table_id
            // << " data_ptr: " << std::hex << meta.data_ptr << " base_off: " << meta.base_off << "
            // bucket_num: " << std::dec << meta.bucket_num << " node_size: " << meta.node_size;
