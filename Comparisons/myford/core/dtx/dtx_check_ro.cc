      ...
      if (likely(fetched_item->valid)) {
        *it = *fetched_item;
        ...
        if (unlikely((it->lock & STATE_INVISIBLE))) {
          std::string event_name = "CheckDirectRO:: State is invisible::Table" + std::to_string(it->table_id);
        ...
        event_counter.RegEvent(t_id, txn_name, "CheckDirectRO:: Item is deleted");
        return false;
      }
    } else {
...
bool DTX::CheckHashRO(std::vector<HashRead>& pending_hash_ro,
  ...
  for (auto& res : pending_hash_ro) {
    ...
    if (likely(find)) {
      ...
      if (unlikely((it->lock & STATE_INVISIBLE))) {
#if INV_ABORT
        event_counter.RegEvent(t_id, txn_name, "CheckHashRO:: Invisible Read");
        return false;
#endif
        // This item is invisible, we need re-read
        char* cas_buf = thread_rdma_buffer_alloc->Alloc(sizeof(lock_t));
      ...
      if (local_hash_node->next == nullptr) {
        std::string table_id =  std::to_string(res.item->item_ptr->table_id);
        std::string event_name = "CheckHashRO:: Key not found::Table" + table_id;
        event_counter.RegEvent(t_id, txn_name, event_name);
        return false;
      }
...
bool DTX::CheckNextHashRO(std::list<InvisibleRead>& pending_invisible_ro,
  ...
  for (auto iter = pending_next_hash_ro.begin(); iter != pending_next_hash_ro.end();) {
    ...
    if (likely(find)) {
      ...
      if (unlikely((it->lock & STATE_INVISIBLE))) {
#if INV_ABORT
        event_counter.RegEvent(t_id, txn_name, "CheckNextHashRO:: Invisible Read");
        return false;
#endif
        // This item is invisible, we need re-read
        char* cas_buf = thread_rdma_buffer_alloc->Alloc(sizeof(lock_t));
