...
bool DTX::CoalescentCommit(coro_yield_t& yield) {
  ...
  *(lock_t*)cas_buf = STATE_INVISIBLE;
  ...
  // (kqh): We embed the thread id into the lock_value, to see if the two 
  // contending transactions are running on the same CN:
  uint64_t lock_value = STATE_LOCKED;
  if (txn_name == "TxPayment") {
    lock_value = PAYMENT_LOCK_VALUE;
  } else if (txn_name == "TxNewOrder") {
    lock_value = NEWORDER_LOCK_VALUE;
  }
  lock_value |= STATE_INVISIBLE;
  lock_value |= ((uint64_t)t_id << 32);
  *(lock_t*)cas_buf = lock_value;
#endif
...
void DTX::Abort() {
  ...
  *((lock_t*)unlock_buf) = 0;
  for (auto& index : locked_rw_set) {
    ...
    // coro_sched->RDMAWrite(coro_id, primary_qp, unlock_buf, it->GetRemoteLockAddr(), sizeof(lock_t));
  }
