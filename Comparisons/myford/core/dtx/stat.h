...
#define MAX_TNUM_PER_CN 120

static bool comparator(const std::pair<std::string, int> &p1,
...
enum KeyType : int { kKeyRead = 0, kKeyWrite, kKeyCommit };

static bool comparator2(const std::pair<T_K, uint64_t> &p1,
                        const std::pair<T_K, uint64_t> &p2) {
  return p1.second > p2.second; // Descending order
}

class EventCount {
public:
  void RegEvent(int t, const std::string &txn_name,
                const std::string &event_name) {
    // if (txn_name == "no") {
    std::string index_name = txn_name + ":" + event_name;
    auto &map = event_cnt[t];
    auto res = map.find(index_name);
    if (res == map.end()) {
      // no such event
      map[index_name] = 1;
    } else {
...
class KeyCount {
public:
  void RegKey(int t, KeyType type, const std::string &txn_name, table_id_t tab,
              itemkey_t k) {
    auto &map =
        (type == kKeyRead)
            ? read_key_cnt[t]
            : ((type == kKeyWrite) ? write_key_cnt[t]
                                   : (type == kKeyCommit ? commit_key_cnt[t]
                                                         : read_key_cnt[t]));
    T_K t_k;
    t_k.table_id = tab;
    t_k.key = k;
  ...
  void Output() {
    ...
    for (auto i : sum_write_key_arr) {
      of << "table: " << i.first.table_id << ", key: " << i.first.key
         << ", cnt: " << i.second << std::endl;
    }
