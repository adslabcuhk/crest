...
#endif
      // uint64_t idx = MurmurHash64A(it->key, 0xdeadbeef) % meta.bucket_num;
      // (kqh): We port motor's hash function here
...
bool DTX::IssueReadLock(std::vector<CasRead> &pending_cas_rw,
  ...
  for (size_t i = 0; i < read_write_set.size(); i++) {
    ...
    if (offset != NOT_FOUND) {
      ...
      // and PaymentTransaction. The thread_id should be embedded into the
      // lock_value
      ...
      doorbell->SetLockReq(cas_buf, it->GetRemoteLockAddr(offset), STATE_CLEAN,
                           lock_value);
      ...
      const HashMeta &meta =
          global_meta_man->GetPrimaryHashMetaWithTableID(it->table_id);
      // uint64_t idx = MurmurHash64A(it->key, 0xdeadbeef) % meta.bucket_num;
      // (kqh): We port the hash function from Motor
      uint64_t idx = GetHash(it->key, meta.bucket_num, meta.hash_core);
      offset_t node_off = idx * meta.node_size + meta.base_off;
      char *local_hash_node = thread_rdma_buffer_alloc->Alloc(sizeof(HashNode));
      if (it->user_insert) {
        for (const auto &t : pending_insert_off_rw) {
          if (t.remote_node == remote_node_id && t.node_off == node_off) {
            need_read = false;
            local_hash_node = t.buf;
            break;
          }
        }
      ...
      if (need_read && !coro_sched->RDMARead(coro_id, qp, local_hash_node,
  ...
  if (!not_eager_locked_rw_set.empty()) {
    ...
    for (auto &index : not_eager_locked_rw_set) {
      ...
      *(lock_t *)cas_buf = 0xdeadbeaf;
      ...
      std::shared_ptr<LockReadBatch> doorbell =
          std::make_shared<LockReadBatch>();
      doorbell->SetLockReq(cas_buf, it->GetRemoteLockAddr(), STATE_CLEAN,
                           STATE_LOCKED);
...
bool DTX::IssueCommitAllBatchSelectFlush(
  ...
  for (auto &set_it : read_write_set) {
    ...
    // Commit primary
    node_id_t node_id =
        set_it.read_which_node; // Read-write data can only be read from primary
    RCQP *qp = thread_qp_man->GetRemoteDataQPWithNodeID(node_id);
    pending_commit_write.push_back(
        CommitWrite{.node_id = node_id, .lock_off = it->GetRemoteLockAddr()});
    std::shared_ptr<InvisibleWriteBatch> doorbell =
