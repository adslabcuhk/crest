...
bool DTX::CheckCasRW(std::vector<CasRead> &pending_cas_rw,
  ...
  // (kqh): Only release locks that is sucessfully acquired
  bool pass_check = true;
#if WORKLOAD_MICRO || WORKLOAD_SMALLBANK || WORKLOAD_TATP
  for (auto &re : pending_cas_rw) {
    if (*((lock_t *)re.cas_buf) != STATE_CLEAN) {
      pass_check = false;
      std::string event_name = "CheckCasRW::Lock is acquired::Table" +
                               std::to_string(re.item->item_ptr->table_id);
      event_counter.RegEvent(t_id, txn_name, event_name);
    } else {
      locked_rw_set.emplace_back(re.idx);
    }
  }
#endif
  ...
  for (auto &re : pending_cas_rw) {
    ...
    if (*((lock_t *)re.cas_buf) != STATE_CLEAN) {
      ...
      if (txn_specific_lock_value == NEWORDER_LOCK_VALUE &&
          txn_name == "TxPayment") {
        std::string event_name = "CheckCasRW:LockFail:"
    ...
    if (likely(fetched_item->key == it->key &&
      ...
      if (it->user_insert) {
        ...
        if (likely(fetched_item->valid)) {
          if (tx_id < fetched_item->version) {
            // std::string event_name = "CheckCasRW::Read Version too
            // New::Table" +
            //                          std::to_string(it->table_id);
            // return false;
            // (kqh): We do not return false, instead, we update the tx_id
            tx_id = fetched_item->version + 1;
          }
      ...
      *((lock_t *)re.cas_buf) = STATE_CLEAN;
      ...
      uint64_t idx = GetHash(it->key, meta.bucket_num, meta.hash_core);
...
int DTX::FindMatchSlot(HashRead &res,
  ...
  for (auto &item : local_hash_node->data_items) {
    if (item.valid && item.key == it->key && item.table_id == it->table_id) {
      if (tx_id < item.version) {
        // return VERSION_TOO_OLD;
        // (kqh): We do not directly return false in this
        tx_id = item.version + 1;
      }
      *it = item;
  ...
  for (auto &res : pending_hash_rw) {
    ...
    if (rc == SLOT_NOT_FOUND) {
      ...
      if (local_hash_node->next == nullptr) {
        std::string event_name = "CheckHashRW:: Slot not found::Table" +
                                 std::to_string(res.item->item_ptr->table_id);
        event_counter.RegEvent(t_id, txn_name, event_name);
        return false;
      }
      ...
      std::string event_name = "CheckHashRW:: Slot not too old::Table" +
                               std::to_string(res.item->item_ptr->table_id);
      event_counter.RegEvent(t_id, txn_name, event_name);
      return false;
    }
...
bool DTX::CheckNextHashRW(std::list<InvisibleRead> &pending_invisible_ro,
  ...
  for (auto iter = pending_next_hash_rw.begin();
    ...
    if (rc == SLOT_FOUND)
      ...
      if (local_hash_node->next == nullptr) {
        event_counter.RegEvent(t_id, txn_name,
                               "CheckNextHashRW:: Item not found");
        return false;
      }
...
bool DTX::CheckInsertOffRW(std::vector<InsertOffRead> &pending_insert_off_rw,
  ...
  for (auto &res : pending_insert_off_rw) {
    ...
    if (rc == OFFSET_NOT_FOUND) {
      auto *local_hash_node = (HashNode *)res.buf;
      if (local_hash_node->next == nullptr) {
        std::string event_name = "CheckInsertOffRW::Slot not found::Table" +
                                 std::to_string(res.item->item_ptr->table_id);
        event_counter.RegEvent(t_id, txn_name, event_name);
        return false;
      }
      ...
      std::string event_name =
          "CheckInsertOffRW::Slot too old or locked::Table" +
          std::to_string(res.item->item_ptr->table_id);
      event_counter.RegEvent(t_id, txn_name, event_name);
      return false;
    }
...
bool DTX::CheckNextOffRW(std::list<InvisibleRead> &pending_invisible_ro,
  ...
  for (auto iter = pending_next_off_rw.begin();
    ...
    if (rc == OFFSET_FOUND) {
      ...
      event_counter.RegEvent(t_id, txn_name,
                             "CheckNextOffRW:: Insert slot not found");
      return false;
    }
  }
  return true;
