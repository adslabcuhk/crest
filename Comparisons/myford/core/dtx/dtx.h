...
class DTX {
  ...
  static constexpr uint64_t PAYMENT_LOCK_VALUE = 0xcdef;

public:
  /************ Interfaces for applications ************/
  void TxBegin(tx_id_t txid, const std::string &name = "");
...
public:
  ...
  uint64_t GetExecTime() { return exec_time; }

  uint64_t GetValidateAndLogTime() { return validate_and_log_time; }

  uint64_t GetCommitTime() { return commit_time; }
...
private:
  ...
  bool
  ExeRW(coro_yield_t &yield); // Execute read-write transaction, use doorbell
...
private:
  ...
  bool
  IssueCommitAllBatchSelectFlush(std::vector<CommitWrite> &pending_commit_write,
                                 char *cas_buf);
...
private:
  ...
  LogOffsetAllocator *thread_remote_log_offset_alloc; // Thread local remote log
                                                      // offset generator
  ...
  uint64_t commit_time = 0;           // The time spent on applying writes
};
...
ALWAYS_INLINE
void DTX::TxBegin(tx_id_t txid, const std::string &name) {
