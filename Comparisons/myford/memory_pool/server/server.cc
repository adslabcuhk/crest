...
void Server::AllocMem() {
    ...
    if (use_pm) {
        ...
        // 0x80003 = MAP_SHARED_VALIDATE | MAP_SYNC. Some old kernel does not have linux/mman.h:
    ...
    offset_t reserve_start =
        hash_buf_size * 0.95;  // reserve 1/4 for hash conflict in case of full bucket
#else
    offset_t reserve_start =
        hash_buf_size * 0.75;  // reserve 1/4 for hash conflict in case of full bucket
#endif
...
void Server::InitRDMA() {
    ...
    RDMA_ASSERT(rdma_ctrl->register_memory(SERVER_HASH_BUFF_ID, hash_buffer, hash_buf_size,
                                           rdma_ctrl->get_device()) == true);
    RDMA_ASSERT(rdma_ctrl->register_memory(SERVER_LOG_BUFF_ID, log_buffer, log_buf_size,
                                           rdma_ctrl->get_device()) == true);
...
void Server::PrepareHashMeta(node_id_t machine_id, std::string& workload, char** hash_meta_buffer,
    ...
    for (auto& hash_table : all_priamry_tables) {
        auto* hash_meta = new HashMeta(hash_table->GetTableID(), (uint64_t)hash_table->GetDataPtr(),
                                       hash_table->GetBucketNum(), hash_table->GetHashNodeSize(),
                                       hash_table->GetBaseOff(), hash_table->GetHashCore());
...
void Server::SendHashMeta(char* hash_meta_buffer, size_t& total_meta_size) {
    ...
    // int from_client_socket = accept(listen_socket, (struct sockaddr*) &client_addr,
...
bool Server::Run() {
    ...
    printf(
        ...
        "==========\n");
    printf(
        "Server now runs as a disaggregated mode. No CPU involvement during RDMA-based transaction "
        "processing\n"
...
int main(int argc, char* argv[]) {
    ...
    auto server = std::make_shared<Server>(machine_id, local_port, local_meta_port, hash_buf_size,
                                           log_buf_size, use_pm, pm_file, mem_size);
    server->AllocMem();
    server->InitMem();
