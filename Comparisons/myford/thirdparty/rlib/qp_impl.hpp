 ...
 */
enum { RC_ID_BASE = 0, UC_ID_BASE = 10000, UD_ID_BASE = 20000 };

inline constexpr uint32_t index_mask() { return 0xffff; }

inline uint32_t mac_mask() { return ::rdmaio::index_mask() << 16; }
...
inline uint32_t decode_qp_mac(uint32_t key) { return (key & ::rdmaio::mac_mask()) >> 16; }

inline uint32_t decode_qp_index(uint32_t key) { return key & ::rdmaio::index_mask(); }
...
class QPImpl {
   public:
    ...
    static ConnStatus poll_till_completion(ibv_cq* cq, ibv_wc& wc, struct timeval timeout) {
        ...
        int poll_result = 0;
        int64_t diff;
        int64_t numeric_timeout = (timeout.tv_sec == 0 && timeout.tv_usec == 0)
...
class RCQPImpl {
   public:
    ...
    static const int RC_MAX_RECV_SIZE =
        1;  // Set to 1 because RC-based two sided verbs are not used
    ...
    static bool readytorcv(ibv_qp* qp, QPAttr& attr, RNicHandler* rnic) {
        auto config = RCConfig{.access_flags = (IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ |
    ...
    static void init(ibv_qp*& qp, ibv_cq*& cq, RNicHandler* rnic) {
        ...
        qp_init_attr.cap.max_recv_wr =
            RC_MAX_RECV_SIZE; /* Can be set to 1, if RC Two-sided is not required */
...
class UDQPImpl {
   public:
    ...
    static void init(ibv_qp*& qp, ibv_cq*& cq, ibv_cq*& recv_cq, RNicHandler* rnic) {
        ...
        if ((recv_cq = ibv_create_cq(rnic->ctx, config.max_recv_size, nullptr, nullptr, 0)) ==
            nullptr) {
    ...
    /**
     * Unlike RC, which change status happens at different places, so F, the function which
     * generates configurations, are passed as templates. On the other hand, UD change status at the
     * same time. So it is more convenient to passed the configuration generated by the F to the
     * functions.
     */
