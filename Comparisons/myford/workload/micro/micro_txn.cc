...
bool TxTestCachedAddr(ZipfGen *zipf_gen, uint64_t *seed, coro_yield_t &yield,
  ...
  //   micro_key.micro_id = 688;  // First read is non-cacheable, set
  //   ATTEMPED_NUM to 5

  //   assert(micro_key.item_key >= 0 && micro_key.item_key < num_keys_global);

  //   micro_objs[i].data_item_ptr =
  //   std::make_shared<DataItem>((table_id_t)MicroTableType::kMicroTable,
  //   micro_key.item_key); micro_objs[i].is_dup = false;
  ...
  //   (micro_val_t*)micro_objs[i].data_item_ptr->value; if (micro_val->magic[0]
  //   != micro_magic) {
...
bool TxReadBackup(ZipfGen *zipf_gen, uint64_t *seed, coro_yield_t &yield,
                  tx_id_t tx_id, DTX *dtx, bool is_skewed,
                  uint64_t data_set_size, uint64_t num_keys_global,
                  uint64_t write_ratio) {
  // This is used to evaluate the performance of reading backup vs. not reading
  // backup when the write ratio is not 0 Remember to set remote_node_id = 0; in
  // dtx_issue.cc to read backup for RO data Use 32 threads and 8 corotines
  // dtx->TxBegin(tx_id);
  // bool is_write[data_set_size];
  // DataItemDuplicate micro_objs[data_set_size];
...
bool TxRFlush1(ZipfGen *zipf_gen, uint64_t *seed, coro_yield_t &yield,
  ...
  //   micro_objs[i] =
  //   std::make_shared<DataItem>((table_id_t)MicroTableType::kMicroTable,
  //   micro_key.item_key); dtx->AddToReadWriteSet(micro_objs[i]);
  // }
...
bool TxRFlush2(ZipfGen *zipf_gen, uint64_t *seed, coro_yield_t &yield,
  ...
  // RDMAReadSync already poll acks std::set<itemkey_t> key_set;
  ...
  //   micro_objs[i] =
  //   std::make_shared<DataItem>((table_id_t)MicroTableType::kMicroTable,
  //   micro_key.item_key); dtx->AddToReadWriteSet(micro_objs[i]); i++;
  // }
...
bool TxUpdate(MICRO *micro_cli, coro_yield_t &yield, tx_id_t tx_id, DTX *dtx,
              UpdateTxnParam *param) {
  dtx->TxBegin(tx_id, "TxUpdate");
  std::vector<DataItemPtr> micro_records;
  for (size_t i = 0; i < param->key_num; ++i) {
    micro_key_t micro_key;
    micro_key.micro_id = param->keys[i];
    table_id_t table_id = 0;
    if (param->keys[i] % 2 == 0) {
      table_id = (table_id_t)MicroTableType::kMicroTable0;
    } else {
      table_id = (table_id_t)MicroTableType::kMicroTable1;
    }
    micro_records.push_back(
        std::make_shared<DataItem>(table_id, micro_key.micro_id));
    dtx->AddToReadWriteSet(micro_records[i]);
  }
  ...
  for (size_t i = 0; i < param->key_num; ++i) {
    auto micro_record = micro_records[i];
    micro_val_t *micro_val = (micro_val_t *)micro_record->value;
    std::string write_value = std::string("txn") + std::to_string(tx_id);
    std::strcpy(micro_val->d1, write_value.c_str());
  }
...
bool TxRead(MICRO *micro_cli, coro_yield_t &yield, tx_id_t tx_id, DTX *dtx,
            ReadTxnParam *param) {
  dtx->TxBegin(tx_id, "TxRead");
  std::vector<DataItemPtr> micro_records;
  for (size_t i = 0; i < param->key_num; ++i) {
    micro_key_t micro_key;
    micro_key.micro_id = param->keys[i];
    ...
    micro_records.push_back(
        std::make_shared<DataItem>(table_id, micro_key.micro_id));
    dtx->AddToReadOnlySet(micro_records[i]);
  }
