...
bool TxAmalgamate(SmallBank *smallbank_client, uint64_t *seed,
  ...
  dtx->TxBegin(tx_id, "TxAmalgamate");

  /* Transaction parameters */
  uint64_t acct_id_0, acct_id_1;
  acct_id_0 = param->custid_0_;
  acct_id_1 = param->custid_1_;

  smallbank_accounts_key_t acc_key_0, acc_key_1;
  acc_key_0.acct_id = acct_id_0;
  acc_key_1.acct_id = acct_id_1;
  auto acc_record_0 = std::make_shared<DataItem>(
      (table_id_t)SmallBankTableType::kAccountsTable, acc_key_0.item_key);
  auto acc_record_1 = std::make_shared<DataItem>(
      (table_id_t)SmallBankTableType::kAccountsTable, acc_key_1.item_key);
  dtx->AddToReadOnlySet(acc_record_0);
  dtx->AddToReadOnlySet(acc_record_1);

  /* Read from savings and checking tables for acct_id_0 */
  smallbank_savings_key_t sav_key_0;
  sav_key_0.acct_id = acct_id_0;
  auto sav_obj_0 = std::make_shared<DataItem>(
      (table_id_t)SmallBankTableType::kSavingsTable, sav_key_0.item_key);
  dtx->AddToReadWriteSet(sav_obj_0);

  smallbank_checking_key_t chk_key_0;
  chk_key_0.acct_id = acct_id_0;
  auto chk_obj_0 = std::make_shared<DataItem>(
      (table_id_t)SmallBankTableType::kCheckingTable, chk_key_0.item_key);
  ...
  auto chk_obj_1 = std::make_shared<DataItem>(
      (table_id_t)SmallBankTableType::kCheckingTable, chk_key_1.item_key);
  ...
  smallbank_savings_val_t *sav_val_0 =
      (smallbank_savings_val_t *)sav_obj_0->value;
  smallbank_checking_val_t *chk_val_0 =
      (smallbank_checking_val_t *)chk_obj_0->value;
  smallbank_checking_val_t *chk_val_1 =
      (smallbank_checking_val_t *)chk_obj_1->value;
...
bool TxBalance(SmallBank *smallbank_client, uint64_t *seed, coro_yield_t &yield,
               tx_id_t tx_id, DTX *dtx, BalanceParam *param) {
  dtx->TxBegin(tx_id, "TxBalance");

  /* Transaction parameters */
  uint64_t acct_id;
  // smallbank_client->get_account(seed, &acct_id);
  acct_id = param->custid_;

  /* Read from accounts tables */
  smallbank_accounts_key_t acc_key;
  acc_key.acct_id = acct_id;
  auto acc_record = std::make_shared<DataItem>(
      (table_id_t)SmallBankTableType::kAccountsTable, acc_key.item_key);
  dtx->AddToReadOnlySet(acc_record);

  /* Read from savings and checking tables */
  smallbank_savings_key_t sav_key;
  sav_key.acct_id = acct_id;
  auto sav_obj = std::make_shared<DataItem>(
      (table_id_t)SmallBankTableType::kSavingsTable, sav_key.item_key);
  dtx->AddToReadOnlySet(sav_obj);

  smallbank_checking_key_t chk_key;
  chk_key.acct_id = acct_id;
  auto chk_obj = std::make_shared<DataItem>(
      (table_id_t)SmallBankTableType::kCheckingTable, chk_key.item_key);
  ...
  smallbank_checking_val_t *chk_val =
      (smallbank_checking_val_t *)chk_obj->value;
...
bool TxDepositChecking(SmallBank *smallbank_client, uint64_t *seed,
  ...
  dtx->TxBegin(tx_id, "TxDepositChecking");
...
bool TxSendPayment(SmallBank *smallbank_client, uint64_t *seed,
  ...
  dtx->TxBegin(tx_id, "TxSendPayment");
...
bool TxTransactSaving(SmallBank *smallbank_client, uint64_t *seed,
  ...
  dtx->TxBegin(tx_id, "TxTransactSaving");
...
bool TxWriteCheck(SmallBank *smallbank_client, uint64_t *seed,
  ...
  dtx->TxBegin(tx_id, "TxWriteCheck");
