...
void SmallBank::LoadTable(node_id_t node_id,
  ...
  if ((node_id_t)SmallBankTableType::kSavingsTable % num_server == node_id) {
    ...
    savings_table = new HashStore((table_id_t)SmallBankTableType::kSavingsTable,
                                  ...
                                  mem_store_alloc_param, HashCore::kDirectFunc);
  ...
  if ((node_id_t)SmallBankTableType::kAccountsTable % num_server == node_id) {
    printf("Primary: Initializing ACCOUNTS table\n");
    // std::string config_filepath = "../../../workload/smallbank/smallbank_tables/checking.json";
    std::string config_filepath = "../../../config/smallbank_config.json";
    auto json_config = JsonConfig::load_file(config_filepath);
    auto table_config = json_config.get("smallbank");
    accounts_table = new HashStore((table_id_t)SmallBankTableType::kAccountsTable,
                                   table_config.get("num_accounts").get_uint64(),
                                   mem_store_alloc_param, HashCore::kDirectFunc);
    PopulateAccountsTable(mem_store_reserve_param);
  ...
  if (BACKUP_DEGREE < num_server) {
    for (node_id_t i = 1; i <= BACKUP_DEGREE; i++) {
      ...
      if ((node_id_t)SmallBankTableType::kAccountsTable % num_server == (node_id - i + num_server) % num_server) {
        printf("Backup: Initializing ACCOUNTS table\n");
        std::string config_filepath = "../../../config/smallbank_config.json";
        auto json_config = JsonConfig::load_file(config_filepath);
        auto table_config = json_config.get("smallbank");
        accounts_table = new HashStore((table_id_t)SmallBankTableType::kAccountsTable,
...
void SmallBank::PopulateSavingsTable(MemStoreReserveParam* mem_store_reserve_param) {
  ...
  RDMA_LOG(INFO) << "total_records_inserted = " << num_accounts_global << "\n";
}
...
void SmallBank::PopulateCheckingTable(MemStoreReserveParam* mem_store_reserve_param) {
  ...
  for (uint32_t acct_id = 0; acct_id < num_accounts_global; acct_id++) {
    ...
    checking_val.bal = 5000000000ull;
...
void SmallBank::PopulateAccountsTable(MemStoreReserveParam* mem_store_reserve_param) {
  for (uint32_t acct_id = 0; acct_id < num_accounts_global; acct_id++) {
    // Accounts
