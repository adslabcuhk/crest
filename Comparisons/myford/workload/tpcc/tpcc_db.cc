...
void TPCC::LoadTable(node_id_t node_id,
  ...
  if ((node_id_t)TPCCTableType::kCustomerTable % num_server == node_id) {
    ...
    customer_table =
        new HashStore((table_id_t)TPCCTableType::kCustomerTable,
                      warehouse_table_config.get("bkt_num").get_uint64() 
                    * district_table_config.get("bkt_num").get_uint64() 
                    * customer_table_config.get("bkt_num").get_uint64() * 1.0,
                      mem_store_alloc_param);
    ...
    history_table =
        new HashStore((table_id_t)TPCCTableType::kHistoryTable,
                      warehouse_table_config.get("bkt_num").get_uint64() 
                    * district_table_config.get("bkt_num").get_uint64() 
                    * customer_table_config.get("bkt_num").get_uint64() * 20.0,
                      mem_store_alloc_param, HashCore::kMurmurFunc);
  ...
  if ((node_id_t)TPCCTableType::kOrderTable % num_server == node_id) {
    ...
    order_table =
        new HashStore((table_id_t)TPCCTableType::kOrderTable,
                      warehouse_table_config.get("bkt_num").get_uint64() 
                    * district_table_config.get("bkt_num").get_uint64() 
                    * customer_table_config.get("bkt_num").get_uint64() * 13.0,
                      mem_store_alloc_param, HashCore::kMurmurFunc);
    order_index_table =
    ...
    new_order_table =
        new HashStore((table_id_t)TPCCTableType::kNewOrderTable,
                    ...
                    * customer_table_config.get("bkt_num").get_uint64() * 13.0,
                      mem_store_alloc_param, HashCore::kMurmurFunc);
    order_line_table =
        // Each district can only have at most 20000 orders, and each bucket has 22 slots to contain
        new HashStore((table_id_t)TPCCTableType::kOrderLineTable,
                      warehouse_table_config.get("bkt_num").get_uint64()
                    * district_table_config.get("bkt_num").get_uint64()
                    * 10000 * 3.0, // Each bucket only contains 5 slots
                      mem_store_alloc_param, HashCore::kTPCCOrderLineFunc);
  ...
  if ((node_id_t)TPCCTableType::kStockTable % num_server == node_id) {
    ...
    auto json_config = JsonConfig::load_file(config_filepath);
    auto table_config = json_config.get("table");
    auto warehouse_config = JsonConfig::load_file(warehouse_config_filepath).get("table");
    uint64_t warehouse_num = warehouse_config.get("bkt_num").get_uint64();
    stock_table =
        new HashStore((table_id_t)TPCCTableType::kStockTable,
                      table_config.get("bkt_num").get_uint64() * warehouse_num / 4,
                      mem_store_alloc_param);
...
void TPCC::PopulateDistrictTable(unsigned long seed, MemStoreReserveParam* mem_store_reserve_param) {
  ...
  uint32_t populate_order_per_district = 1;
  FastRandom random_generator(seed);
  for (uint32_t w_id = 1; w_id <= num_warehouse; w_id++) {
    for (uint32_t d_id = 1; d_id <= num_district_per_warehouse; d_id++) {
      ...
      district_val.d_next_o_id = populate_order_per_district + 1;
      //  NOTICE:: scale should check consistency requirements.
  ...
  for (uint32_t w_id = 1; w_id <= num_warehouse; w_id++) {
    for (uint32_t d_id = 1; d_id <= num_district_per_warehouse; d_id++) {
      for (uint32_t c_id = 1; c_id <= num_customer_per_district; c_id++) {
        ...
        // //TODO:: MakeCustomerIndexKey may have some problem even the same <w_id, d_id, c_last, c_first> will cause different customer_index_key
...
void TPCC::PopulateOrderNewOrderAndOrderLineTable(unsigned long seed, MemStoreReserveParam* mem_store_reserve_param) {
  ...
  for (uint32_t w_id = 1; w_id <= num_warehouse; w_id++) {
    for (uint32_t d_id = 1; d_id <= num_district_per_warehouse; d_id++) {
      ...
      while (c_ids.size() != populate_order_per_district) {
      ...
      for (uint32_t c = 1; c <= populate_order_per_district; c++) {
