...
struct tpcc_customer_val_t {
  ...
  static const int MIN_DATA = 50;  // C_DATA random a-string [300 .. 500]
...
static_assert(sizeof(tpcc_customer_val_t) == 264, "");
...
struct tpcc_order_line_val_t {
  ...
  static const int MAX_OL_CNT = 14;
...
static_assert(sizeof(tpcc_order_line_val_t) == 48, "");  // add debug magic
...
static_assert(sizeof(tpcc_stock_val_t) == 208, "");  // add debug magic
...
#define FREQUENCY_PAYMENT 46
#define FREQUENCY_ORDER_STATUS 4
#define FREQUENCY_DELIVERY 0
#define FREQUENCY_STOCK_LEVEL 0
#else
...
struct NewOrderParam {
  ...
  int64_t local_stocks[tpcc_order_line_val_t::MAX_OL_CNT];
  int64_t remote_item_ids[tpcc_order_line_val_t::MAX_OL_CNT];
  int64_t local_item_ids[tpcc_order_line_val_t::MAX_OL_CNT];
  uint32_t local_supplies[tpcc_order_line_val_t::MAX_OL_CNT];
  uint32_t remote_supplies[tpcc_order_line_val_t::MAX_OL_CNT];
};
...
class TPCC {
 public:
  ...
  int64_t MakeCustomerKey(int32_t w_id, int32_t d_id, int32_t c_id) {
    ...
    uint64_t id = upper_id * num_customer_per_district + c_id;
  ...
  int64_t MakeOrderLineKey(int32_t w_id, int32_t d_id, int32_t o_id, int32_t number) {
    ...
    // (kqh): 20000 is the max numerber of order each district can have
    int64_t oid = static_cast<int64_t>(upper_id) * 10000 + static_cast<int64_t>(o_id);
