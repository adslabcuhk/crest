...
bool TxNewOrder(TPCC *tpcc_client, FastRandom *random_generator,
  ...
  */
  ...
  std::vector<DataItemPtr> item_records;
  std::vector<DataItemPtr> stock_records;
  std::vector<DataItemPtr> orderline_records;

  std::vector<DataItemPtr> remote_item_records;
  std::vector<DataItemPtr> remote_stock_records;
  std::vector<DataItemPtr> remote_orderline_records;
  ...
  for (int ol_number = 1; ol_number <= num_local_stocks; ol_number++) {
    ...
    item_records.push_back(item_obj);
    ...
    stock_records.push_back(stock_obj);
  }
  ...
  for (int ol_number = 1; ol_number <= num_remote_stocks; ol_number++) {
    ...
    auto item_obj = std::make_shared<DataItem>(
        (table_id_t)TPCCTableType::kItemTable, tpcc_item_key.item_key);
    dtx->AddToReadOnlySet(item_obj);
    remote_item_records.push_back(item_obj);
    ...
    auto stock_obj = std::make_shared<DataItem>(
        (table_id_t)TPCCTableType::kStockTable, stock_key.item_key);
    dtx->AddToReadWriteSet(stock_obj);
    remote_stock_records.push_back(stock_obj);
  }
  ...
  if (check != tpcc_zip_magic) {
    RDMA_LOG(FATAL) << "[FATAL] Read district unmatch, tid-cid-txid: "
                    << dtx->t_id << "-" << dtx->coro_id << "-" << tx_id;
  }
  ...
  // (kqh): Remove the OrderIndex table, we don't need it
  ...
  for (int ol_number = 1; ol_number <= num_local_stocks; ol_number++) {
    ...
    orderline_records.push_back(ol_obj);
  }
  ...
  for (int ol_number = 1; ol_number <= num_remote_stocks; ol_number++) {
    ...
    auto ol_obj = std::make_shared<DataItem>(
        ...
        sizeof(tpcc_order_line_val_t), order_line_key.item_key, tx_id, 1);
    dtx->AddToReadWriteSet(ol_obj);
    remote_orderline_records.push_back(ol_obj);
  }
  ...
  // Processing the Order Table related:
  ...
  for (int ol_number = 1; ol_number <= num_local_stocks; ol_number++) {
    DataItemPtr item_obj = item_records[ol_number - 1];
    DataItemPtr stock_obj = stock_records[ol_number - 1];
    DataItemPtr ol_obj = orderline_records[ol_number - 1];
  ...
  for (int ol_number = 1; ol_number <= num_remote_stocks; ol_number++) {
    ...
    DataItemPtr item_obj = remote_item_records[ol_number - 1];
    DataItemPtr stock_obj = remote_stock_records[ol_number - 1];
    DataItemPtr ol_obj = remote_orderline_records[ol_number - 1];
...
bool TxOrderStatus(TPCC *tpcc_client, FastRandom *random_generator,
  ...
  */
  ...
  tpcc_order_val_t *order_val = (tpcc_order_val_t *)order_obj->value;
  if (order_val->o_entry_d == 0) {
    RDMA_LOG(FATAL) << "[FATAL] Read order unmatch, tid-cid-txid: " << dtx->t_id
                    << "-" << dtx->coro_id << "-" << tx_id;
  }
