...
bool TxGetSubsciberData(TATP *tatp_client, uint64_t *seed, coro_yield_t &yield,
                        tx_id_t tx_id, DTX *dtx, GetSubscriberTxnParam *param) {
  // RDMA_LOG(DBG) << "coro " << dtx->coro_id << " executes TxGetSubsciberData,
  // tx_id=" << tx_id;
  dtx->TxBegin(tx_id, "GetSubscriberData");
...
bool TxGetNewDestination(TATP *tatp_client, uint64_t *seed, coro_yield_t &yield,
  ...
  // TxGetNewDestination";
  dtx->TxBegin(tx_id, "GetNewDestination");
  ...
  if (!dtx->TxExe(yield)) {
    event_counter.RegEvent(dtx->t_id, "GetNewDestination", "TxExeFail");
    return false;
  }
  ...
  if (callfwd_success) {
    ...
    dtx->TxAbortReadOnly();
    event_counter.RegEvent(dtx->t_id, "GetNewDestination", "TxAbortReadOnly");
    return false;
  }
}
...
bool TxGetAccessData(TATP *tatp_client, uint64_t *seed, coro_yield_t &yield,
                     tx_id_t tx_id, DTX *dtx, GetAccessDataTxnParam *param) {
  // RDMA_LOG(DBG) << "coro " << dtx->coro_id << " executes TxGetAccessData,
  // tx_id=" << tx_id;
  dtx->TxBegin(tx_id, "GetAccessData");
...
bool TxUpdateSubscriberData(TATP *tatp_client, uint64_t *seed,
  ...
  dtx->TxBegin(tx_id, "UpdateSubscriberData");
  ...
  // specfac_key.s_id = s_id;
  // specfac_key.sf_type = sf_type;
  specfac_key.s_id = param->s_id_;
  specfac_key.sf_type = param->sf_type_;
...
bool TxUpdateLocation(TATP *tatp_client, uint64_t *seed, coro_yield_t &yield,
                      tx_id_t tx_id, DTX *dtx, UpdateLocationTxnParam *param) {
  // RDMA_LOG(DBG) << "coro " << dtx->coro_id << " executes TxUpdateLocation,
  // tx_id=" << tx_id;

  dtx->TxBegin(tx_id, "UpdateLocation");
...
bool TxInsertCallForwarding(TATP *tatp_client, uint64_t *seed,
  ...
  dtx->TxBegin(tx_id, "InsertCallForwarding");
...
bool TxDeleteCallForwarding(TATP *tatp_client, uint64_t *seed,
  ...
  dtx->TxBegin(tx_id, "DeleteCallForwarding");
