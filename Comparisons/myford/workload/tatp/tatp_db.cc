...
void TATP::LoadTable(node_id_t node_id, node_id_t num_server,
  ...
  if ((node_id_t)TATPTableType::kSecSubscriberTable % num_server == node_id) {
    ...
    auto config_filepath =
        "../../../workload/tatp/tatp_tables/sec_subscriber.json";
    auto json_config = JsonConfig::load_file(config_filepath);
    auto table_config = json_config.get("table");
    sec_subscriber_table = new HashStore(
  ...
  if ((node_id_t)TATPTableType::kAccessInfoTable % num_server == node_id) {
    ...
    auto json_config = JsonConfig::load_file(config_filepath);
    auto table_config = json_config.get("table");
    access_info_table = new HashStore(
  ...
  if ((node_id_t)TATPTableType::kSpecialFacilityTable % num_server == node_id) {
    ...
    special_facility_table = new HashStore(
    ...
    call_forwarding_table = new HashStore(
        (table_id_t)TATPTableType::kCallForwardingTable,
        table_config2.get("bkt_num").get_uint64(), mem_store_alloc_param);
...
void TATP::PopulateSubscriberTable(
  ...
  for (uint32_t s_id = 0; s_id < subscriber_size; s_id++) {
    ...
    total_records_inserted += LoadRecord(
        subscriber_table, key.item_key, (void *)&sub_val,
        sizeof(tatp_sub_val_t), (table_id_t)TATPTableType::kSubscriberTable,
        mem_store_reserve_param);
    total_records_examined++;
    // std::cerr << "total_records_examined: " << total_records_examined <<
    // std::endl;
  }
}
...
void TATP::PopulateAccessInfoTable(
  ...
  for (uint32_t s_id = 0; s_id < subscriber_size; s_id++) {
    std::vector<uint8_t> ai_type_vec = ai_type_values;
        // SelectUniqueItem(&tmp_seed, ai_type_values, 1, 4);
    for (uint8_t ai_type : ai_type_vec) {
      ...
      total_records_inserted += LoadRecord(
          access_info_table, key.item_key, (void *)&accinf_val,
          sizeof(tatp_accinf_val_t),
          (table_id_t)TATPTableType::kAccessInfoTable, mem_store_reserve_param);
      total_records_examined++;
    }
  }
}
...
int TATP::LoadRecord(HashStore *table, itemkey_t item_key, void *val_ptr,
