...
class JsonConfig {
 public:
  ...
  JsonConfig(const std::shared_ptr<rapidjson::Document> &root, rapidjson::Value *current,
             std::string path);
...
JsonConfig JsonConfig::empty_array(std::string path) {
  std::shared_ptr<rapidjson::Document> root = std::make_shared<rapidjson::Document>();
  root->SetArray();
...
JsonConfig JsonConfig::load_file(std::string path) {
  ...
  conf.assign((std::istreambuf_iterator<char>(ifs)), std::istreambuf_iterator<char>());
...
JsonConfig JsonConfig::load(std::string json_text, std::string path) {
  std::shared_ptr<rapidjson::Document> root = std::make_shared<rapidjson::Document>();
  root->Parse<rapidjson::ParseFlag::kParseDefaultFlags | rapidjson::ParseFlag::kParseCommentsFlag>(
      json_text.c_str());
...
JsonConfig JsonConfig::get(size_t index) {
  ...
  if (index >= current_->Size())
    ...
    return JsonConfig(root_, &((*current_)[static_cast<unsigned int>(index)]), oss.str());
}
...
ALWAYS_INLINE
JsonConfig::JsonConfig(const std::shared_ptr<rapidjson::Document> &root, rapidjson::Value *current,
                       std::string path)
