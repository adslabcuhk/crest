...
class FordComputeNode(Node):
    def __init__(self, ip: str, id: int):
        super().__init__(ip, id)
        self.project_path = config.fordPath
        self.bin_path = self.project_path + "/build/compute_pool/run/"
        if id == 0:
            self.numa_node = 0
        else:
            self.numa_node = 1
    ...
    def exec_cmd(self, cmd: str) -> bool:
        # To prevent the compute node runs too long, we also set the timeout parameter
        # The default value is set to be 600s, which should be enough for the benchmark
        # to run
        stdin, stdout, stderr = self.ssh.exec_command(cmd, timeout=300.0, get_pty=True)
        timeout = config.execCmdTimeout
        endtime = time.time() + timeout
        long_running = False
        while not stdout.channel.exit_status_ready():
    ...
    def get_results(self, workload: str, dst_dir: str):
        ...
        dst_path = dst_dir + "/ford_{}_results_cn{}.txt".format(workload, self.id)
        with SCPClient(self.ssh.get_transport()) as scp:
            scp.get(src_path, dst_path)

        src_path = self.project_path + "/bench_results/{}/abort_rate.txt".format(
            workload
        )
        dst_path = dst_dir + "/ford_{}_abort_rate_cn{}.txt".format(workload, self.id)
        with SCPClient(self.ssh.get_transport()) as scp:
            scp.get(src_path, dst_path)
...
class FordBenchmark:
    def __init__(
        ...
        self.memory_pool = []
        for n in mns:
            self.memory_pool.append(FordMemoryNode(n[0], n[1]))

        self.compute_pool = []
        for n in cns:
            self.compute_pool.append(FordComputeNode(n[0], n[1]))

    def clean_bench_results(self):
        for n in self.compute_pool:
            # Remove previous results
            config_cmd = "rm -rf {}/bench_results/{}".format(
                n.project_path, self.workload
            )
    ...
    def build_project(self):
        ...
        for n in self.compute_pool + self.memory_pool:
            reset_command = r"sed -i 's/\(#define WORKLOAD_[A-Z]\+\) 1/\1 0/g' {}/core/flags.h".format(
                n.project_path
            )
            n.exec_cmd(reset_command)
            if self.workload == "tpcc":
                config_cmd = r"sed -i 's/#define WORKLOAD_TPCC 0/#define WORKLOAD_TPCC 1/' {}/core/flags.h".format(
                    n.project_path
                )
                n.exec_cmd(config_cmd)
            if self.workload == "smallbank":
                config_cmd = r"sed -i 's/#define WORKLOAD_SMALLBANK 0/#define WORKLOAD_SMALLBANK 1/' {}/core/flags.h".format(
                    n.project_path
                )
                n.exec_cmd(config_cmd)
            if self.workload == "micro":
                config_cmd = r"sed -i 's/#define WORKLOAD_MICRO 0/#define WORKLOAD_MICRO 1/' {}/core/flags.h".format(
                    n.project_path
                )
                n.exec_cmd(config_cmd)
    ...
    def setup_tpcc_configuration(self, warehouse_num: int):
        self.num_warehouse = warehouse_num

        # Change the configuration file for both memory pool and compute pool
        # Setup their warehouse number
        for n in self.memory_pool:
            config_file_path = (
                n.project_path + "/" + "workload/tpcc/tpcc_tables/warehouse.json"
            )
            config_cmd = 'sed -i \'s/"bkt_num": [0-9]*/"bkt_num": {}/g\' {}'.format(
                warehouse_num, config_file_path
            )
            n.exec_cmd(config_cmd)

            # Also set the memory_node_config.json file of Ford:
            # Change the memory reserved and the workload name:
            config_file_path = n.project_path + "/" + "config/memory_node_config.json"
            config_cmd = (
            ...
            mr_size = int(170 / 100 * warehouse_num) + 1
            config_cmd = (
                'sed -i \'s/"mem_size_GB": [0-9]*/"mem_size_GB": {}/g\' {}'.format(
    ...
    def setup_smallbank_configuration(self, account_num: int, theta: float):
        self.num_accounts = account_num
        self.theta = theta
        ...
        for n in self.memory_pool:
            ...
            config_file_path = n.project_path + "/" + "config/memory_node_config.json"
            config_cmd = (
                'sed -i \'s/"workload": "[^"]*"/"workload": "SmallBank"/\' {}'.format(
                    config_file_path
                )
            )
            n.exec_cmd(config_cmd)
    ...
    def create_detail_dir(self, thread_num: int, coro_num: int):
        if self.workload == "tpcc":
            self.detail_dir = "ford_tpcc_detail_warehouse_{}_threads_{}_coro_{}".format(
                self.num_warehouse, thread_num, coro_num
            )
        if self.workload == "smallbank":
            self.detail_dir = "ford_smallbank_detail_accounts_{}_zipfian_{}_threads_{}_coro_{}".format(
                self.num_accounts, self.theta, thread_num, coro_num
            )
        if self.workload == "micro":
            self.detail_dir = "ford_micro_detail_keys_{}_zipfian_{}_op_{}_wr_{}_threads_{}_coro_{}".format(
                self.num_key,
                self.theta,
                self.num_op,
                self.write_ratio,
    ...
    def print_run_header(self, thread_num: int, coro_num: int):
        if self.workload == "tpcc":
            print(
                "[=] Run Ford TPCC Benchmark: {} Threads, {} Coros".format(
                    thread_num, coro_num
                )
            )
        if self.workload == "smallbank":
            print(
                "[=] Run Ford SmallBank Benchmark: {} Threads, {} Coros".format(
                    thread_num, coro_num
                )
            )
        if self.workload == "micro":
            print(
                "[=] Run Ford Micro Benchmark: {} Threads, {} Coros".format(
                    thread_num, coro_num
                )
            )
    ...
    def launch_compute_pool(self, thread_num: int, coro_num: int):
        ...
        def spawn_run(n: FordComputeNode):
        ...
        if self.workload == "tpcc":
            res = (
                "Ford {} Benchmark: Warehouse[{}], Threads[{}], Coroutines[{}]".format(
                    self.workload, self.num_warehouse, thread_num, coro_num
                )
            )
        if self.workload == "smallbank":
            res = "Ford {} Benchmark: Accounts[{}], Zipfian[{}], Threads[{}], Coroutines[{}]".format(
                self.workload, self.num_accounts, self.theta, thread_num, coro_num
            )
        if self.workload == "micro":
            res = "Ford {} Benchmark: Keys[{}], Zipfian[{}], Op[{}], Threads[{}], Coroutines[{}]".format(
                self.workload,
                self.num_key,
                self.theta,
                self.num_op,
    ...
    def process_results(self):
        all_data = []
        for n in self.compute_pool:
            fname = "ford_{}_results_cn{}.txt".format(self.workload, n.id)
        ...
        with open("ford_{}_aggregated_thpt".format(self.workload), "w") as f:
            column_headers = [
                "tried_thpt(KOPS)",
                "committed_thpt",
                "avg_latency(us)",
