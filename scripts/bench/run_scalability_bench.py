...
def run_motor_scalability(workload: str):
    ...
    if workload == "smallbank":
        ...
        wait_load_time = 30
    if workload == "micro":
        bench.setup_micro_configuration(NUM_KEYS, ZIPFIAN, NUM_OP)
        wait_load_time = 30

    # Thread number increase from 4 to 40, the number of coordinators
...
def run_ford_scalability(workload: str):
...
if __name__ == "__main__":
    ...
    if sysname == "crest":
        ...
        run_ford_scalability(workload)
    elif sysname == "motor":
        run_motor_scalability(workload)
    else:
        print("Unknown system name: {}".format(sysname))
